...
import { createBasicRoadmap, deleteOwnedRoadmap, getOwnedRoadmap, listOwnedRoadmaps, smartChangeRoadmap, updateOwnedRoadmap } from "../../../api/roadmap-service.ts";
...
async function roadmapAction(auth: any, action: string, body: any) {
  ...
  if (action === "list_roadmaps") return { roadmaps: await listOwnedRoadmaps(client, auth.userId) };
  if (action === "roadmap_questions") {
    const category = typeof body.category === "string" ? body.category : "skill_learning";
    const presets: Record<string, any[]> = {
      exam_preparation: [{ id: "goal", question: "What result are you aiming for?", type: "multiline", required: true, placeholder: "Example: 90%+ in my half-yearly exam." }, { id: "deadline", question: "When is the exam?", type: "text", required: true, placeholder: "Example: 20 September" }, { id: "baseline", question: "What is your current level?", type: "multiline", required: true }, { id: "time", question: "How much time can you study on a normal day?", type: "number", required: true }],
      chess: [{ id: "goal", question: "What chess result are you targeting?", type: "multiline", required: true, placeholder: "Example: reach 1500 rapid." }, { id: "baseline", question: "What is your current rating and biggest weakness?", type: "multiline", required: true }, { id: "time", question: "How many minutes can you train per day?", type: "number", required: true }],
      academics: [{ id: "goal", question: "What academic result do you want?", type: "multiline", required: true }, { id: "deadline", question: "When is the deadline or exam?", type: "text", required: true }, { id: "baseline", question: "Where are you starting from?", type: "multiline", required: true }],
    };
    return { questions: presets[category] || [{ id: "goal", question: "What result are you aiming for?", type: "multiline", required: true, placeholder: "Describe the destination in concrete terms." }, { id: "deadline", question: "What deadline are you working toward?", type: "text", required: true }, { id: "baseline", question: "What is your current starting point?", type: "multiline", required: true }, { id: "time", question: "How much time can you commit most days?", type: "number", required: true }] };
  }
  if (action === "delete_roadmap") return deleteOwnedRoadmap(client, auth.userId, String(body.roadmapId || ""));
  if (action === "update_roadmap") return updateOwnedRoadmap(client, auth.userId, String(body.roadmapId || ""), { title: body.title, goal: body.goal });
  if (action === "smart_change") return smartChangeRoadmap(client, auth.userId, String(body.roadmapId || ""), String(body.request || ""));
  if (action === "generate_roadmap") {
    const category = typeof body.category === "string" ? body.category : "skill_learning";
    const answers = body.answers && typeof body.answers === "object" ? body.answers : {};
    return createBasicRoadmap(client, auth.userId, category, answers);
  }
...
serve(async (req) => {
  ...
  try {
    ...
    const result = streamText({ model: provider.model, system: buildSystemPrompt(body?.appContext), messages: await convertToModelMessages(uiMessages.slice(-12)), tools: createProductivityTools(auth.client as any, auth.userId, auth.token), stopWhen: stepCountIs(5), maxOutputTokens: 700, maxRetries: 0, abortSignal: req.signal, onError: (error) => console.error("[outstand-ai] provider error", error) });
    return result.toUIMessageStreamResponse({ originalMessages: uiMessages.slice(-12), headers: { ...corsHeaders, "Cache-Control": "no-cache, no-transform", "X-Accel-Buffering": "no" }, onError: (error) => { console.error("[outstand-ai] stream failure", error); const message = error instanceof Error ? error.message : String(error); if (/429|quota|rate[_ -]?limit|resource[_ -]?exhausted/i.test(message)) return "AI provider is temporarily rate-limited. Please retry shortly."; return "AI request failed. Please try again."; } });
  } catch (error) {
