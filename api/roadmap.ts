...
async function auth(req: VercelRequest) {
  const token = tokenOf(req); if (!token) throw Object.assign(new Error("Authentication required."), { status: 401 });
  ...
  const { data, error } = await client.auth.getUser(token); if (error || !data.user) throw Object.assign(new Error("Authentication failed."), { status: 401 });
...
async function callAI(prompt: string) {
  ...
  if (groq) {
    const response = await fetch("https://api.groq.com/openai/v1/chat/completions", { method: "POST", headers: { Authorization: `Bearer ${groq}`, "Content-Type": "application/json" }, body: JSON.stringify({ model: "llama-3.1-8b-instant", temperature: 0.15, max_tokens: 5000, response_format: { type: "json_object" }, messages: [{ role: "system", content: rules }, { role: "user", content: prompt }] }) });
    const raw = await response.text();
    if (response.ok) { const parsed = JSON.parse(raw); const content = parsed?.choices?.[0]?.message?.content; if (typeof content === "string" && content.trim()) return parseJson(content); }
  ...
  const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent`, { method: "POST", headers: { "Content-Type": "application/json", "x-goog-api-key": google }, body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: `${rules}\n\n${prompt}` }] }], generationConfig: { responseMimeType: "application/json", temperature: 0.15 } }) });
...
function validatePlan(plan: any) {
  ...
  for (const milestone of plan.milestones) { if (!Number.isInteger(milestone.day) || typeof milestone.title !== "string" || typeof milestone.outcome !== "string" || !Array.isArray(milestone.actions) || milestone.actions.length < 1 || milestone.actions.length > 3) throw new Error("The AI returned an invalid milestone."); for (const action of milestone.actions) if (typeof action.title !== "string" || typeof action.instructions !== "string" || !Number.isInteger(action.estimatedMinutes) || action.estimatedMinutes < 10 || action.estimatedMinutes > 90 || typeof action.successCriteria !== "string") throw new Error("The AI returned an invalid action."); }
...
function normalizePlan(plan: any) {
  plan.milestones = plan.milestones.slice(0, 6).map((m: any) => ({ ...m, actions: m.actions.slice(0, 3) }));
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (mode === "plan") {
      ...
      const plan = validatePlan(await callAI(`Build a concise execution plan for this ${category} goal. Duration: ${durationDays} days. User answers: ${JSON.stringify(answers)}. User context: ${JSON.stringify(context)}.\n\nCreate 3-6 meaningful milestones for the whole goal, not a chapter-by-chapter syllabus. Each milestone has 1-3 actions. Generate at most 3 schedule blocks per day, only when useful and within the user's real availability. Each block must tell the user exactly what to do and how success is measured. The first day must contain 1-3 actions in a top-level today array. Do not fill empty time with generic tasks. Include at most 4 metrics and one short adaptationRule.\n\nReturn exactly {"title":"","summary":"","durationDays":${durationDays},"difficulty":"","assumptions":[],"milestones":[{"day":1,"title":"","outcome":"","actions":[{"title":"","instructions":"","estimatedMinutes":30,"taskType":"practice","methodologyTags":[],"resources":[],"spacedRepetitionDay":null,"successCriteria":""}]}],"dailySchedule":[{"day":1,"startTime":"18:00","endTime":"18:30","title":"","instructions":"","taskType":"practice","methodologyTags":[],"estimatedMinutes":30,"successCriteria":"","resources":[]}],"today":[""],"metrics":[""],"adaptationRule":""}`));
      return json(res, 200, { plan: normalizePlan(plan) });
    }
    ...
    const changed = JSON.stringify(adapted) !== JSON.stringify(saved.plan);
    if (changed) { const { error } = await client.from("ai_roadmaps").update({ plan: adapted, updated_at: new Date().toISOString() }).eq("id", saved.id).eq("user_id", userId); if (error) throw Object.assign(new Error(`Could not save the adapted roadmap: ${error.message}`), { status: 500 }); }
    return json(res, 200, { plan: adapted, changed, reason: changed ? "Your plan was adjusted using today's progress." : "Your current plan still fits your progress." });
