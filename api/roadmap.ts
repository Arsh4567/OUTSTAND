...
function validatePlan(plan: any) {
  ...
  if (!plan.summary || typeof plan.summary !== "string") throw new Error("The AI returned a plan without a clear outcome summary.");
  if (!plan.northStar || typeof plan.northStar.metric !== "string" || typeof plan.northStar.baseline !== "string" || typeof plan.northStar.target !== "string") throw new Error("The AI returned a plan without a measurable north-star target.");
  for (const milestone of plan.milestones) {
    if (!Number.isInteger(milestone.day) || typeof milestone.title !== "string" || typeof milestone.outcome !== "string" || typeof milestone.exitCriteria !== "string" || !milestone.exitCriteria.trim() || !Array.isArray(milestone.actions) || milestone.actions.length < 1 || milestone.actions.length > 3 || milestone.actions.some((action: unknown) => typeof action !== "string" || !action.trim())) throw new Error("The AI returned an invalid milestone.");
  }
...
function normalizePlan(plan: any) {
  plan.milestones = plan.milestones.slice(0, 6).map((m: any) => ({ ...m, actions: m.actions.slice(0, 3), exitCriteria: String(m.exitCriteria).slice(0, 240) }));
  plan.dailySchedule = plan.dailySchedule.slice(0, Math.max(1, Math.min(plan.durationDays, 90)));
  plan.today = Array.isArray(plan.today) ? plan.today.slice(0, 3) : plan.dailySchedule.filter((b: any) => b.day === 1).slice(0, 3).map((b: any) => b.title);
  plan.metrics = Array.isArray(plan.metrics) ? plan.metrics.slice(0, 4) : [];
  plan.assumptions = Array.isArray(plan.assumptions) ? plan.assumptions.slice(0, 4) : [];
  plan.recoveryRule = typeof plan.recoveryRule === "string" ? plan.recoveryRule.slice(0, 400) : "Protect the deadline: keep high-value work, reduce optional work, and resume from the next unfinished checkpoint.";
  plan.northStar = { metric: String(plan.northStar.metric).slice(0, 120), baseline: String(plan.northStar.baseline).slice(0, 120), target: String(plan.northStar.target).slice(0, 160) };
  return plan;
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (mode === "plan") {
      ...
      const plan = validatePlan(await callAI(`Build a highly practical, personalized execution roadmap for this ${category} goal. Duration: ${durationDays} days. Answers: ${JSON.stringify(answers)}. Context: ${JSON.stringify(context)}.${syllabusPrompt(category, answers)}\n\nDesign principles:\n- Start from the user's baseline and target, not from generic curriculum categories.\n- Choose the smallest number of high-leverage phases that can realistically move the user toward the target.\n- Every milestone must have a concrete outcome and a pass/fail exit criterion.\n- Every daily block must produce evidence: a completed artifact, solved set, tested recall, scored attempt, reflection, or other observable result.\n- Use deliberate practice, retrieval, spaced repetition, interleaving, feedback, or error correction only where they fit the goal.\n- Build at least one checkpoint into the plan where progress is measured against the north-star target.\n- Include a recoveryRule describing what should be protected when the user misses work; never blindly stack all missed tasks onto the next day.\n- Keep the daily schedule realistic: at most 3 meaningful blocks per day, with clear timing and no filler.\n- Preserve academic/exam scope as a hard boundary.\n\nReturn exactly this JSON shape: {"title":"","summary":"One-sentence outcome","northStar":{"metric":"","baseline":"","target":""},"durationDays":${durationDays},"difficulty":"","assumptions":[],"recoveryRule":"","milestones":[{"day":1,"title":"","outcome":"","exitCriteria":"A measurable pass condition"}],"dailySchedule":[{"day":1,"startTime":"18:00","endTime":"18:30","title":"","instructions":"","taskType":"practice","methodologyTags":[],"estimatedMinutes":30,"successCriteria":"Observable evidence of completion","resources":[]}],"today":[],"metrics":[],"adaptationRule":""}`));
