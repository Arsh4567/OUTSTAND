...
function isRateLimit(status: number, text: string) { return status === 429 || /rate.?limit|quota|too many requests|resource.?exhausted/i.test(text); }
...
async function askGemini(prompt: string): Promise<AIJsonResult> {
  ...
  if (!response.ok) {
    let detail = "";
    try { const parsed = JSON.parse(raw); detail = parsed?.error?.message || ""; } catch { detail = raw.slice(0, 300); }
    const error = new Error(`Gemini request failed (${response.status})${detail ? `: ${detail}` : "."}`);
...
async function ask(prompt: string) {
  try { return await askGroq(prompt); } catch (error: any) {
    if ((isRateLimit(error?.status || 0, error?.message || "") || error?.code === "GROQ_REQUEST_FAILED") && env("GEMINI_API_KEY", "GOOGLE_GENERATIVE_AI_API_KEY", "GOOGLE_API_KEY")) return await askGemini(prompt);
    throw error;
  }
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (mode === "questions") {
      ...
      const result = await ask(`Create the next 3 to 5 highest-value questions for a ${category} goal. Do not repeat answered information. Universal requirements: collect the exact goal, measurable target, timeline/duration and difficulty preference before planning. For academics, if the target involves Class 10/board exams, collect class, board, target percentage/grade, subjects, current marks or baseline, exam date/days remaining, school/tuition schedule, realistic weekday/weekend availability, weak chapters and strong chapters. Ask for study windows if a timetable will depend on them. Return exactly {"questions":[{"id":"string","question":"string","type":"text|number|choice|multiline","required":true,"options":["..."],"placeholder":"..."}].\n\nAlready answered:\n${answered}\nSelected habits:\n${JSON.stringify(habits)}\nUser context:\n${JSON.stringify(context)}`);
      return json(res, 200, result);
    }
    if (mode === "plan") {
      const result = await ask(`Build a genuinely personalized ${category} roadmap from the complete interview below. This is the execution engine, not a motivational template. Return exactly {"plan":{"title":"string","summary":"string","durationDays":number,"difficulty":"string","assumptions":["string"],"milestones":[{"day":number,"title":"string","outcome":"string","actions":["string"]}],"today":["string"],"timetable":[{"label":"string","durationMinutes":number,"task":"string","why":"string"}],"metrics":["string"],"adaptationRule":"string"}}. Hard requirements: directly serve the user's stated outcome; every action must be concrete, measurable and connected to the goal; never fill with generic wellness actions unless explicitly selected; for Class 10 exams use actual subjects/topics, revision, question practice, mocks, error review and exam strategy; today's actions must name the actual subject/topic or a defined diagnostic/practice task; timetable must fit stated school/tuition and available time with 3 to 6 realistic blocks; if required information is missing return {"needsMoreInfo":true,"questions":[...]}; milestones cover the full duration; Day 1 must have useful actions; selected habits support rather than replace goal work.\n\nCategory: ${category}\nAnswers:\n${JSON.stringify(answers)}\nSelected habits:\n${JSON.stringify(habits)}\nUser context:\n${JSON.stringify(context)}`);
      return json(res, 200, result);
    }
