...
function normalizePlan(plan: any) { plan.milestones = plan.milestones.slice(0, 6).map((m: any) => ({ ...m, actions: m.actions.slice(0, 3) })); plan.dailySchedule = plan.dailySchedule.slice(0, Math.max(1, Math.min(plan.durationDays, 180))); plan.today = Array.isArray(plan.today) ? plan.today.slice(0, 3) : plan.dailySchedule.filter((b: any) => b.day === 1).slice(0, 3).map((b: any) => b.title); plan.metrics = Array.isArray(plan.metrics) ? plan.metrics.slice(0, 6) : []; plan.assumptions = Array.isArray(plan.assumptions) ? plan.assumptions.slice(0, 6) : []; return plan; }
function syllabusPrompt(category: string, answers: Record<string, unknown>) { if (!["exam_preparation", "academics"].includes(category)) return ""; return `\n\nACADEMIC SCOPE CONTRACT:\n- classOrGrade: ${JSON.stringify(answers.classOrGrade || answers.class || answers.grade)}\n- examOrBoard: ${JSON.stringify(answers.boardOrExam || answers.examName || answers.exam)}\n- examDate: ${JSON.stringify(answers.examDate || answers.deadline)}\n- syllabusScope: ${JSON.stringify(answers.syllabusScope || answers.examCoverage || answers.coverage)}\nTreat declared syllabus boundaries as hard constraints. Never add unconfirmed chapters or topics.`; }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (mode === "questions") {
      ...
      return json(res, 200, await callAI(`Create only the next 3-6 highest-value intake questions for a ${category} goal. Never repeat answered information. Collect the minimum information needed for an executable roadmap. Always establish: (1) exact outcome and measurable definition of success, (2) current level/baseline, (3) deadline, (4) realistic available time and fixed commitments, (5) constraints. ${examMode ? "Also require class/grade, board or exam name, exam date/deadline, syllabus boundary, and known weak areas. Keep the syllabus question bounded: ask for included chapters/units or a short scope statement, not a request to paste an entire textbook." : "For non-academic goals, baseline can be a level, prior experience, current output, or recent result."} Do not ask for preferences unless they materially affect scheduling. Return {"questions":[{"id":"string","question":"string","type":"text|number|choice|multiline","required":true,"options":[],"placeholder":""}]}.\nAnswered:\n${answered}\nContext:\n${JSON.stringify(context)}`, { maxTokens: 1500, maxOutputTokens: 1500 }));
    }
    ...
    if (mode === "plan") {
      ...
      const plan = validatePlan(await callAI(`Build an execution-first roadmap for this ${category} goal. Duration: ${durationDays} days. Answers: ${JSON.stringify(answers)}. Context: ${JSON.stringify(context)}.${syllabusPrompt(category, answers)} Rules: work backward from the deadline; each milestone must have a measurable outcome, not a generic learning phase; every schedule block needs a concrete success criterion; schedule a small number of high-value actions; include testing and repair when the goal supports it; do not spend time on filler; respect fixed commitments and available time. For academic/exam goals, never schedule outside the declared scope. Return at most 6 milestones and at most 3 blocks per day. Include metrics that could later explain progress (examples: accuracy, attempts, time, checkpoint score, confidence, consistency). Include an adaptationRule explaining what deterministic logic should do after completion, skip, or struggle. Return JSON: {"title":"","summary":"","durationDays":${durationDays},"difficulty":"","assumptions":[],"milestones":[{"day":1,"title":"","outcome":"Measurable result","actions":["Concrete action"]}],"dailySchedule":[{"day":1,"startTime":"18:00","endTime":"18:30","title":"","instructions":"","taskType":"practice","methodologyTags":[],"estimatedMinutes":30,"successCriteria":"A measurable definition of done","resources":[]}],"today":[],"metrics":[{"name":"accuracy","measurement":"..."}],"adaptationRule":"..."}`));
    ...
    if (mode === "edit") {
      ...
      const lower = request.toLowerCase(); const current = { title: String(roadmap.title || ""), goal: String(roadmap.goal || ""), durationDays: Number(roadmap.duration_days) || 30 };
      ...
      if (titleMatch?.[1]) { const title = titleMatch[1].trim().replace(/[.!?]+$/, "").slice(0, 120); const { error } = await client.from("roadmaps").update({ title }).eq("id", roadmapId).eq("user_id", userId); if (error) throw error; return json(res, 200, { changed: true, message: "Roadmap title updated." }); }
      if (goalMatch?.[1]) { const goal = goalMatch[1].trim().replace(/[.!?]+$/, "").slice(0, 2000); const { error } = await client.from("roadmaps").update({ goal }).eq("id", roadmapId).eq("user_id", userId); if (error) throw error; return json(res, 200, { changed: true, message: "Roadmap goal updated." }); }
      if (daysMatch && /\b(?:day|days|duration|shorten|extend|longer|shorter)\b/i.test(request)) { const durationDays = Math.max(1, Math.min(730, Number(daysMatch[1]))); const startDate = new Date(); const targetDate = new Date(startDate); targetDate.setDate(startDate.getDate() + durationDays - 1); const { error } = await client.from("roadmaps").update({ duration_days: durationDays, target_date: targetDate.toISOString().slice(0, 10) }).eq("id", roadmapId).eq("user_id", userId); if (error) throw error; if (durationDays < current.durationDays) await client.from("roadmap_tasks").update({ is_required: false }).eq("roadmap_id", roadmapId).eq("user_id", userId).gt("day_number", durationDays); return json(res, 200, { changed: true, message: `Roadmap duration changed to ${durationDays} days.` }); }
      if (/\b(?:after|from|at)\s+\d{1,2}(?::\d{2})?\s*(?:am|pm)?\b/i.test(lower) && /\b(?:study|session|sessions|schedule|tasks|blocks|move|shift|evening)\b/i.test(lower)) { const time = request.match(/\b(?:after|from|at)\s+(\d{1,2})(?::(\d{2}))?\s*(am|pm)?\b/i); if (time) { let hour = Number(time[1]); const minute = Number(time[2] || 0); const meridiem = time[3]?.toLowerCase(); if (meridiem === "pm" && hour < 12) hour += 12; if (meridiem === "am" && hour === 12) hour = 0; const start = hour * 60 + minute; const { data: tasks, error: taskError } = await client.from("roadmap_tasks").select("id,start_time,end_time").eq("roadmap_id", roadmapId).eq("user_id", userId).order("day_number").order("task_order"); if (taskError) throw taskError; let cursor = start; for (const task of tasks || []) { const s = task.start_time && /^\d{2}:\d{2}$/.test(task.start_time) ? minutesOf(task.start_time) : null; const e = task.end_time && /^\d{2}:\d{2}$/.test(task.end_time) ? minutesOf(task.end_time) : null; if (s === null || e === null || e <= s) continue; const blockMinutes = Math.max(10, e - s); await client.from("roadmap_tasks").update({ start_time: timeString(cursor), end_time: timeString(cursor + blockMinutes) }).eq("id", task.id).eq("user_id", userId); cursor += blockMinutes + 15; if (cursor >= 23 * 60 + 30) break; } return json(res, 200, { changed: true, message: `Schedule moved to start after ${timeString(start)}.` }); } }
      const compact = await callAI(`Return the smallest possible structured patch for this roadmap edit. Do not regenerate the roadmap. Allowed patch fields: title, goal, durationDays, startTime, endTime. Existing: ${JSON.stringify(current)}. User request: ${request}. If the request needs a genuine plan rewrite, return {"changed":false,"reason":"needs_full_plan"}. Otherwise return only changed fields.`, { maxTokens: 500, maxOutputTokens: 500 });
      if (!compact?.changed || !compact.patch || typeof compact.patch !== "object") return json(res, 200, { changed: false, message: "That change needs a larger roadmap rewrite, so it was not applied automatically." });
      const patch: Record<string, unknown> = {}; if (typeof compact.patch.title === "string") patch.title = compact.patch.title.slice(0, 120); if (typeof compact.patch.goal === "string") patch.goal = compact.patch.goal.slice(0, 2000); if (Number.isFinite(Number(compact.patch.durationDays))) patch.duration_days = Math.max(1, Math.min(730, Number(compact.patch.durationDays))); if (!Object.keys(patch).length) return json(res, 200, { changed: false, message: "No safe roadmap fields were changed." }); if (patch.duration_days) { const startDate = new Date(); const targetDate = new Date(startDate); targetDate.setDate(startDate.getDate() + Number(patch.duration_days) - 1); patch.target_date = targetDate.toISOString().slice(0, 10); } const { error: updateError } = await client.from("roadmaps").update(patch).eq("id", roadmapId).eq("user_id", userId); if (updateError) throw updateError; return json(res, 200, { changed: true, message: typeof compact.message === "string" ? compact.message : "Roadmap updated." });
    }

    return json(res, 400, { error: "Unsupported roadmap mode." });
