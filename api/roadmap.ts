...
async function callGroq(prompt: string) {
  ...
  if (!response.ok) {
    let detail = "";
    try { const parsed = JSON.parse(raw); detail = typeof parsed?.error?.message === "string" ? `: ${parsed.error.message}` : ""; } catch { /* ignore malformed provider error */ }
    throw Object.assign(new Error(`Groq request failed (${response.status})${detail}`), { status: response.status });
  }
...
function addMinutes(value: string, delta: number) { return timeString(minutesOf(value) + delta); }
function normalizeDayOneSchedule(plan: any, currentMinutes: number, availabilityStart?: string, availabilityEnd?: string) {
  ...
  const remaining: any[] = [];
  for (const block of dayOne) {
    const duration = Math.max(10, Number(block.estimatedMinutes) || Math.max(10, minutesOf(block.endTime) - minutesOf(block.startTime)));
    const originalEnd = minutesOf(block.endTime);
    if (originalEnd <= currentMinutes) continue;
    if (cursor + duration > endCeiling) break;
    block.startTime = timeString(cursor);
    block.endTime = timeString(cursor + duration);
    remaining.push(block);
    cursor += duration + 15;
  }
  const ids = new Set(remaining);
  plan.dailySchedule = blocks.filter((block: any) => Number(block.day) !== 1 || ids.has(block));
  plan.today = remaining.slice(0, 3).map((block: any) => block.title);
  if (remaining.length === 0) plan.today = ["Start tomorrow — Day 1 has finished for today."];
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (mode === "questions") {
      ...
      return json(res, 200, await callAI(`Create only the next 3-5 highest-value questions for a ${category} goal. Never repeat answered information. Collect only what is needed to build an executable plan. ${examMode ? "This is an academic/exam goal. Ask for: class/grade, board or exam name, exam date/deadline, and how much syllabus is included. Do NOT ask the user to paste the entire syllabus. Make syllabus coverage easy to answer, e.g. full syllabus, first half, specific chapters/topics, or a short text description. Also collect normal weekday availability/fixed commitments and baseline only if not already known." : "Collect exact outcome and measurable target, current baseline, deadline, normal weekday availability/fixed commitments, preferred session length, and important constraints."} If already known, do not ask again. Return {"questions":[{"id":"string","question":"string","type":"text|number|choice|multiline","required":true,"options":[],"placeholder":""}]} .\n\nAnswered:\n${answered}\nContext:\n${JSON.stringify(context)}`));
    }
    if (mode === "plan") {
      ...
      const plan = validatePlan(await callAI(`Build a concise execution plan for this ${category} goal. Duration: ${durationDays} days. User answers: ${JSON.stringify(answers)}. User context: ${JSON.stringify(context)}.${syllabusPrompt(category, answers)}\n\nCreate 3-6 meaningful milestones for the whole goal, not a chapter-by-chapter syllabus. Each milestone has 1-3 short action strings. Generate at most 3 schedule blocks per day, only when useful and within the user's real availability. Each block must tell the user exactly what to do and how success is measured. The first day must contain 1-3 actions in a top-level today array, but its times are provisional because OUTSTAND will roll Day 1 forward from the user's current local time. Do not fill empty time with generic tasks. Include at most 4 metrics and one short adaptationRule.\n\nReturn exactly {"title":"","summary":"","durationDays":${durationDays},"difficulty":"","assumptions":[],"milestones":[{"day":1,"title":"","outcome":"","actions":["Do one concrete thing","Practice one concrete thing"]}],"dailySchedule":[{"day":1,"startTime":"18:00","endTime":"18:30","title":"","instructions":"","taskType":"practice","methodologyTags":[],"estimatedMinutes":30,"successCriteria":"","resources":[]}],"today":[""],"metrics":[""],"adaptationRule":""}`));
      normalizeDayOneSchedule(plan, new Date().getHours() * 60 + new Date().getMinutes(), availabilityStart, availabilityEnd);
