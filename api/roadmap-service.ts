...
function dayNameForTask(startDate: string, dayNumber: number) {
  const date = new Date(`${startDate}T00:00:00`);
  date.setDate(date.getDate() + Math.max(1, dayNumber) - 1);
  return date.toLocaleDateString("en-US", { weekday: "long" }).toLowerCase();
}

function taskMatches(task: any, subject: string) {
  const needle = subject.trim().toLowerCase().replace(/\s+/g, " ");
  if (!needle) return false;
  const haystack = `${task.title || ""} ${task.instructions || ""} ${task.success_criteria || ""} ${task.task_type || ""}`.toLowerCase();
  const tokens = needle.split(/[^a-z0-9]+/).filter((token: string) => token.length >= 3);
  return haystack.includes(needle) || (tokens.length > 0 && tokens.every((token: string) => haystack.includes(token)));
}

function parseWeekday(request: string) {
  const days = ["sunday", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday"];
  const found = days.find((day) => new RegExp(`\\b${day}s?\\b`, "i").test(request));
  return found || null;
}

function parseSubject(request: string) {
  const patterns = [
    /(?:remove|delete|drop|cancel)\s+(.+?)(?:\s+on\s+(?:monday|tuesday|wednesday|thursday|friday|saturday|sunday)s?)?(?:\s+from\s+the\s+roadmap)?$/i,
    /(?:replace|swap)\s+(.+?)\s+with\s+(.+?)(?:\s+on\s+(?:monday|tuesday|wednesday|thursday|friday|saturday|sunday)s?)?$/i,
    /(?:rename|change)\s+(?:the\s+)?task\s+(.+?)\s+(?:to|as)\s+(.+)$/i,
    /(?:move|shift)\s+(.+?)\s+(?:to|at)\s+(\d{1,2})(?::(\d{2}))?\s*(am|pm)?$/i,
  ];
...
async function deleteTasks(client: any, userId: string, roadmapId: string, tasks: any[]) {
  for (const task of tasks) {
    const { error } = await client.from("roadmap_tasks").delete().eq("id", task.id).eq("roadmap_id", roadmapId).eq("user_id", userId);
    if (error) throw error;
  }
}
...
export async function smartChangeRoadmap(client: any, userId: string, roadmapId: string, request: string) {
  ...
  if (!tasks.length) throw new Error("This roadmap has no tasks to change.");

  const weekday = parseWeekday(normalized);
  const semantic = parseSubject(normalized);
  const removeMatch = normalized.match(/^(?:remove|delete|drop|cancel)\s+(.+?)(?:\s+on\s+(?:monday|tuesday|wednesday|thursday|friday|saturday|sunday)s?)?(?:\s+from\s+the\s+roadmap)?$/i);
  if (removeMatch) {
    const subject = removeMatch[1].trim();
    const matches = tasks.filter((task: any) => taskMatches(task, subject) && (!weekday || dayNameForTask(roadmap.start_date, Number(task.day_number)) === weekday));
    if (!matches.length) throw new Error(`No roadmap tasks matched “${subject}”${weekday ? ` on ${weekday}s` : ""}.`);
    await deleteTasks(client, userId, roadmapId, matches);
    const remaining = await ownedTasks(client, userId, roadmapId);
    if (remaining.some((task: any) => matches.some((deleted: any) => deleted.id === task.id))) throw new Error("Task deletion could not be verified.");
    return { changed: true, roadmapId, action: "delete_tasks", affectedTasks: matches.length, verified: true };
  }

  const replaceMatch = normalized.match(/^(?:replace|swap)\s+(.+?)\s+with\s+(.+?)(?:\s+on\s+(?:monday|tuesday|wednesday|thursday|friday|saturday|sunday)s?)?$/i);
  if (replaceMatch) {
    const oldSubject = replaceMatch[1].trim();
    const replacement = replaceMatch[2].trim();
    const matches = tasks.filter((task: any) => taskMatches(task, oldSubject) && (!weekday || dayNameForTask(roadmap.start_date, Number(task.day_number)) === weekday));
    if (!matches.length) throw new Error(`No roadmap tasks matched “${oldSubject}”${weekday ? ` on ${weekday}s` : ""}.`);
    for (const task of matches) {
      const { error } = await client.from("roadmap_tasks").update({ title: replacement.slice(0, 200) }).eq("id", task.id).eq("roadmap_id", roadmapId).eq("user_id", userId);
      if (error) throw error;
    }
    const verify = await ownedTasks(client, userId, roadmapId);
    if (matches.some((old: any) => !verify.some((next: any) => next.id === old.id && next.title === replacement.slice(0, 200)))) throw new Error("Task replacement could not be verified.");
    return { changed: true, roadmapId, action: "replace_tasks", affectedTasks: matches.length, verified: true };
  }

  const renameTask = normalized.match(/^(?:rename|change)\s+(?:the\s+)?task\s+(.+?)\s+(?:to|as)\s+(.+)$/i);
  if (renameTask) {
    const subject = renameTask[1].trim();
    const title = renameTask[2].trim().replace(/[.!?]+$/, "").slice(0, 200);
    const matches = tasks.filter((task: any) => taskMatches(task, subject));
    if (!matches.length) throw new Error(`No roadmap task matched “${subject}”.`);
    if (matches.length > 1) throw new Error(`Multiple tasks matched “${subject}”. Please specify the day or exact task name.`);
    const { error } = await client.from("roadmap_tasks").update({ title }).eq("id", matches[0].id).eq("roadmap_id", roadmapId).eq("user_id", userId);
    if (error) throw error;
    const verify = await ownedTasks(client, userId, roadmapId);
    const updated = verify.find((task: any) => task.id === matches[0].id);
    if (!updated || updated.title !== title) throw new Error("Task rename could not be verified.");
    return { changed: true, roadmapId, action: "rename_task", affectedTasks: 1, verified: true };
  }

  const taskTime = normalized.match(/^(?:move|shift)\s+(.+?)\s+(?:to|at)\s+(\d{1,2})(?::(\d{2}))?\s*(am|pm)?(?:\s+on\s+(?:monday|tuesday|wednesday|thursday|friday|saturday|sunday)s?)?$/i);
  if (taskTime) {
    const subject = taskTime[1].trim();
    let hour = Number(taskTime[2]);
    const minute = Number(taskTime[3] || 0);
    const meridiem = taskTime[4]?.toLowerCase();
    if (meridiem === "pm" && hour < 12) hour += 12;
    if (meridiem === "am" && hour === 12) hour = 0;
    if (hour > 23 || minute > 59) throw new Error("Invalid target time.");
    const matches = tasks.filter((task: any) => taskMatches(task, subject) && (!weekday || dayNameForTask(roadmap.start_date, Number(task.day_number)) === weekday));
    ...
    for (const task of matches) {
      ...
      const duration = start !== null && end !== null ? Math.max(1, end - start) : Number(task.estimated_minutes) || 30;
      const { error } = await client.from("roadmap_tasks").update({ start_time: fromMinutes(hour * 60 + minute), end_time: fromMinutes(hour * 60 + minute + duration) }).eq("id", task.id).eq("roadmap_id", roadmapId).eq("user_id", userId);
      if (error) throw error;
    }
    return { changed: true, roadmapId, action: "move_tasks", affectedTasks: matches.length, verified: true };
  }
  ...
  if (timeShift === null && targetTime === null && dayShift === null) throw new Error("I could not identify a safe roadmap change. Try naming the task and requested change, for example: remove Chemistry on Fridays, replace Algebra with Revision, rename Chemistry to Organic Chemistry, or move Physics to 7 PM.");
  ...
  const updates = tasks.map((task: any) => {
    ...
    if (targetTime !== null && start !== null) { const delta = targetTime - start; patch.start_time = fromMinutes(start + delta); if (end !== null) patch.end_time = fromMinutes(end + delta); }
  ...
  return { changed: true, roadmapId, action: timeShift !== null ? "shift_time" : targetTime !== null ? "move_to_time" : "shift_days", affectedTasks: updates.length, request: normalized, verified: true };
}
