...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (mode !== "plan") {
      const base = `${req.headers["x-forwarded-proto"] || "https"}://${req.headers.host}/api/roadmap`; const upstream = await fetch(base, { method: "POST", headers: { Authorization: `Bearer ${bearer(req)}`, "Content-Type": "application/json" }, body: JSON.stringify(input) }); const text = await upstream.text(); return res.status(upstream.status).setHeader("Content-Type", "application/json").send(text);
    }
    ...
    const upstreamUrl = `${req.headers["x-forwarded-proto"] || "https"}://${req.headers.host}/api/roadmap`; const upstream = await fetch(upstreamUrl, { method: "POST", headers: { Authorization: `Bearer ${bearer(req)}`, "Content-Type": "application/json" }, body: JSON.stringify(input) });
