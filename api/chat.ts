...
const env = (...names: string[]) => names.map((name) => process.env[name]).find(Boolean);
...
function textFromMessage(message: any) {
  if (Array.isArray(message?.parts)) {
    return message.parts.filter((part: any) => part?.type === "text" && typeof part.text === "string").map((part: any) => part.text).join("");
  }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  if (req.method === "OPTIONS") {
    res.status(204).setHeader("Access-Control-Allow-Origin", "*").setHeader("Access-Control-Allow-Headers", "Authorization, Content-Type").setHeader("Access-Control-Allow-Methods", "GET, POST, DELETE, OPTIONS").end();
    return;
  }
  ...
  if (req.method === "GET") {
    ...
    const geminiConfigured = Boolean(env("GEMINI_API_KEY", "GOOGLE_GENERATIVE_AI_API_KEY"));
  ...
  try {
    ...
    const uiMessages: UIMessage[] = rawMessages.map((message: any) => ({
      id: typeof message?.id === "string" ? message.id : crypto.randomUUID(),
      role: message?.role === "assistant" ? "assistant" : "user",
      parts: [{ type: "text", text: textFromMessage(message).trim() }],
    })).filter((message) => message.parts[0].text.length > 0) as UIMessage[];
    ...
    if (!apiKey) { sendJson(res, 503, { error: "Gemini API configuration is missing.", code: "GEMINI_CONFIG_MISSING" }); return; }
