...
export function createProductivityTools(client: Db, userId: string, accessToken: string): ToolSet {
  return {
    get_memory: tool({
      description: "Read a small set of relevant saved OUTSTAND AI memories such as user preferences, stable constraints, active goals, successful strategies, or recurring patterns. Use when personalization materially benefits the response. Do not request the entire memory store.",
      inputSchema: jsonSchema<{ query?: string }>({ type: "object", properties: { query: { type: "string", description: "Optional topic such as study schedule, chess goals, preferred planning style, or constraints" } }, additionalProperties: false }),
      execute: async ({ query }) => {
        const { data, error } = await client.from("ai_memory").select("memory_type,memory_key,memory_value,confidence,updated_at").eq("user_id", userId).order("updated_at", { ascending: false }).limit(12);
        if (error) throw error;
        const rows = data || [];
        if (!query?.trim()) return { memories: rows };
        const tokens = query.toLowerCase().split(/[^a-z0-9]+/).filter((token: string) => token.length >= 3);
        const filtered = rows.filter((row: any) => tokens.some((token: string) => `${row.memory_key} ${row.memory_value}`.toLowerCase().includes(token))).slice(0, 8);
    ...
    save_memory: tool({
      ...
      execute: async ({ memoryType, memoryKey, memoryValue, confidence }) => {
        const forbidden = /(password|passcode|token|secret|api[_ -]?key|credit card|bank account|social security|otp)/i;
        if (forbidden.test(memoryKey) || forbidden.test(memoryValue)) throw new Error("That information cannot be stored as AI memory.");
        const { data: existing, error: lookupError } = await client.from("ai_memory").select("id,memory_value").eq("user_id", userId).eq("memory_key", memoryKey.trim()).maybeSingle();
        if (lookupError && lookupError.code !== "PGRST116") throw lookupError;
        const payload = { user_id: userId, memory_type: memoryType, memory_key: memoryKey.trim().slice(0, 80), memory_value: memoryValue.trim().slice(0, 500), confidence: typeof confidence === "number" ? confidence : 0.8, source: "assistant" };
        const { data, error } = await client.from("ai_memory").upsert(payload, { onConflict: "user_id,memory_key" }).select("memory_type,memory_key,memory_value,confidence,updated_at").single();
        if (error) throw error;
        if (!data) throw new Error("AI memory was not persisted.");
    ...
    delete_memory: tool({
      ...
      execute: async ({ memoryKey }) => {
        const { data, error } = await client.from("ai_memory").delete().eq("user_id", userId).eq("memory_key", memoryKey.trim()).select("memory_key").maybeSingle();
        if (error) throw error;
    ...
    mark_habit: tool({
      ...
      execute: async ({ habitId, completed }) => {
        const { data, error } = await client.from("user_productivity_state").select("habits").eq("user_id", userId).maybeSingle(); if (error) throw error;
        const current = Array.isArray(data?.habits) ? data.habits : []; const index = current.findIndex((habit: any) => habit?.id === habitId); if (index < 0) return { changed: false, message: "That habit does not exist." };
        const history = Array.isArray(current[index]?.history) ? current[index].history : []; const nextHistory = completed ? Array.from(new Set([...history, today()])) : history.filter((day: string) => day !== today());
    ...
    set_task_progress: tool({
      ...
      execute: async ({ taskId, status }) => {
        const { data: task, error: taskError } = await client.from("roadmap_tasks").select("id,roadmap_id,title").eq("id", taskId).eq("user_id", userId).maybeSingle(); if (taskError) throw taskError;
        if (!task) return { changed: false, message: "That task does not exist." };
        const { data: existing, error: existingError } = await client.from("roadmap_task_progress").select("status").eq("task_id", taskId).eq("user_id", userId).maybeSingle(); if (existingError && existingError.code !== "PGRST116") throw existingError;
        if (existing?.status === "completed" && status !== "completed") return { changed: false, message: "Completed tasks are protected. Explicitly ask to reopen this task if needed." };
        const { error } = await client.from("roadmap_task_progress").upsert({ task_id: taskId, roadmap_id: task.roadmap_id, user_id: userId, status, completed_at: status === "completed" ? new Date().toISOString() : null }, { onConflict: "task_id,user_id" }); if (error) throw error;
        const { data: verify, error: verifyError } = await client.from("roadmap_task_progress").select("status,completed_at").eq("task_id", taskId).eq("user_id", userId).maybeSingle(); if (verifyError) throw verifyError;
        if (!verify || verify.status !== status) throw new Error("Task update could not be verified.");
    ...
    change_roadmap: tool({
      ...
      execute: async ({ roadmapId, request }) => {
        const base = process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : "http://localhost:3000"; const response = await fetch(`${base}/api/roadmap-edit`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${accessToken}` }, body: JSON.stringify({ roadmapId, request }) });
        const result = await response.json(); if (!response.ok) throw new Error(result.error || "Could not change the roadmap.");
        const { data: verify, error: verifyError } = await client.from("roadmap_tasks").select("id,day_number,start_time,end_time,title").eq("roadmap_id", roadmapId).eq("user_id", userId).order("day_number").order("task_order").limit(8); if (verifyError) throw verifyError;
    ...
    set_reminder: tool({
      ...
      execute: async ({ title, body, time, category, daysOfWeek }) => {
        ...
        const { data, error } = await client.from("notification_jobs").insert({ user_id: userId, category, title: title.trim().slice(0, 160), body: body.trim().slice(0, 500), local_time: time, timezone, days_of_week: Array.isArray(daysOfWeek) && daysOfWeek.length ? daysOfWeek : [0, 1, 2, 3, 4, 5, 6], enabled: true }).select("id,title,local_time,timezone,days_of_week").single(); if (error) throw error;
        const { data: verify, error: verifyError } = await client.from("notification_jobs").select("id,title,local_time,timezone,days_of_week,enabled").eq("id", data.id).eq("user_id", userId).maybeSingle(); if (verifyError) throw verifyError; if (!verify) throw new Error("Reminder could not be verified.");
        return { created: true, reminder: verify, verified: true };
      },
    }),
  };
}
