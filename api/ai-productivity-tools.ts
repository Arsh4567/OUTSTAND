...
import { buildDailyWorkloadInstructions, validateDailyWorkload } from "./daily-workload-planner.js";
...
const strategySchema = {
  type: "object",
  properties: {
    primaryMethod: { type: "string", minLength: 3, maxLength: 160 },
    methods: { type: "array", items: { type: "string", minLength: 2, maxLength: 100 }, minItems: 1, maxItems: 8 },
    rationale: { type: "string", minLength: 10, maxLength: 600 },
    progression: { type: "string", minLength: 10, maxLength: 600 },
    practiceAndFeedback: { type: "string", minLength: 10, maxLength: 600 },
    reviewCadence: { type: "string", minLength: 3, maxLength: 240 },
    adaptationRule: { type: "string", minLength: 10, maxLength: 600 },
  },
  required: ["primaryMethod", "methods", "rationale", "progression", "practiceAndFeedback", "reviewCadence", "adaptationRule"],
...
const taskSchema = {
  ...
  properties: {
    ...
    taskType: { type: "string", minLength: 2, maxLength: 40 },
    methodologyTags: { type: "array", items: { type: "string", minLength: 2, maxLength: 80 }, maxItems: 6 },
    successCriteria: { type: "string", minLength: 5, maxLength: 500 },
    resources: { type: "array", items: { type: "string", minLength: 1, maxLength: 300 }, maxItems: 5 },
    guidance: { type: "object", additionalProperties: true },
    isRequired: { type: "boolean" },
  },
  required: ["title", "instructions", "estimatedMinutes", "taskType", "methodologyTags", "successCriteria"],
  additionalProperties: false,
};
...
const dailyWorkloadSchema = {
  type:"object", properties:{
    availableMinutes:{type:"integer",minimum:15,maximum:720},
    reason:{type:"string",minLength:5,maxLength:300}
  }, required:["availableMinutes"], additionalProperties:false
};
...
export function createProductivityTools(client: Db, userId: string, _accessToken: string): ToolSet {
  ...
  tools.create_roadmap = tool({
    description: `Create a canonical roadmap by planning backward from a measurable outcome. ${buildBackwardPlanningInstructions()} ${buildDailyWorkloadInstructions()} Return a complete backwardPlan and dailyWorkload before creating the roadmap. Every milestone must lead toward the target, include task focus, and fit within the user's daily capacity.`,
    inputSchema: jsonSchema({
      type:"object", properties:{
        ...
        dailyWorkload:dailyWorkloadSchema,
      ...
      }, required:["goal","goalSpec","strategy","backwardPlan","dailyWorkload"], additionalProperties:false
    }),
    execute:async(args:any)=>{
      const goal=String(args.goal||"").trim(); if(goal.length<5) throw new Error("A roadmap goal is required.");
      const goalSpec={outcome:String(args.goalSpec?.outcome||"").trim().slice(0,500),metric:String(args.goalSpec?.metric||"").trim().slice(0,160),target:String(args.goalSpec?.target||"").trim().slice(0,160),baseline:String(args.goalSpec?.baseline||"unknown").trim().slice(0,160)||"unknown",deadline:String(args.goalSpec?.deadline||"").trim().slice(0,40)||null,constraints:Array.isArray(args.goalSpec?.constraints)?args.goalSpec.constraints.map((v:unknown)=>String(v).trim().slice(0,200)).filter(Boolean).slice(0,10):[]};
      ...
      const availableMinutes=Math.max(15,Math.min(720,Number(args.dailyWorkload?.availableMinutes)||Number(args.answers?.dailyTimeMinutes)||Number(args.answers?.availableMinutes)||60));
      const questionnaire={...(args.answers||{}),goalSpec,dailyWorkload:{availableMinutes}};
      const generationMetadata={source:"assistant",phase:"daily-workload",goalSpec,strategy,backwardPlan,dailyWorkload:{availableMinutes}};
      ...
      const workloadTasks=plan.milestones.flatMap((milestone:any)=>Array.isArray(milestone.tasks)&&milestone.tasks.length?milestone.tasks.map((task:any)=>({day:Number(milestone.day)||1,estimatedMinutes:Number(task.estimatedMinutes)||30})):Array.isArray(milestone.actions)?milestone.actions.map(()=>({day:Number(milestone.day)||1,estimatedMinutes:30})):[]);
      const dailyWorkload=validateDailyWorkload(workloadTasks,duration,availableMinutes);
      ...
      return {created:true,roadmapId:data,verified:true,goalSpec,strategy,backwardPlan,dailyWorkload,message:"Roadmap created with a measurable outcome, backward plan, and daily workload limit."};
    },
  });

  tools.get_today = tool({ description:"Read today's roadmap tasks. Never guess a roadmap when multiple roadmaps exist.", inputSchema:jsonSchema({type:"object",properties:{roadmapId:{type:"string"},roadmapTitle:{type:"string"}},additionalProperties:false}), execute:async(args:any)=>{const roadmap=await resolveRoadmap(client,userId,args.roadmapId,args.roadmapTitle);const day=dayNumber(roadmap.start_date,roadmap.duration_days);const {data:tasks,error}=await client.from("roadmap_tasks").select("id,title,day_number,task_order,start_time,end_time,estimated_minutes,is_required,task_type,instructions,success_criteria,methodology_tags,resources,guidance").eq("roadmap_id",roadmap.id).eq("user_id",userId).eq("day_number",day).order("task_order");if(error)throw error;return{roadmap,todayDay:day,tasks:tasks||[],verified:true};} });

  tools.list_habits = tool({description:"List the user's current habits and today's completion state.",inputSchema:jsonSchema({type:"object",properties:{},additionalProperties:false}),execute:async()=>{const state=await readState(client,userId);const date=today();return{habits:state.habits.map((h:any)=>({...h,completedToday:Array.isArray(h?.completedDates)&&h.completedDates.includes(date)})),verified:true};}});
  tools.mark_habit = tool({description:"Mark or unmark a habit for today. Only use when explicitly requested.",inputSchema:jsonSchema({type:"object",properties:{habitId:{type:"string"},habitName:{type:"string"},completed:{type:"boolean"}},required:["completed"],additionalProperties:false}),execute:async(args:any)=>{const{state,habit}=await findHabit(client,userId,args.habitId,args.habitName);const date=today();const completedDates=Array.isArray(habit.completedDates)?[...habit.completedDates]:[];const index=completedDates.indexOf(date);if(args.completed&&index<0)completedDates.push(date);if(!args.completed&&index>=0)completedDates.splice(index,1);await writeState(client,{...state,habits:state.habits.map((i:any)=>i.id===habit.id?{...habit,completedDates}:i)});const verified=await findHabit(client,userId,habit.id);const complete=Array.isArray(verified.habit.completedDates)&&verified.habit.completedDates.includes(date);if(complete!==args.completed)throw new Error("Habit completion could not be verified.");return{changed:true,habit:verified.habit,completedToday:complete,verified:true};}});
  tools.update_habit = tool({description:"Update a habit's name, frequency, or reminder without losing completion history.",inputSchema:jsonSchema({type:"object",properties:{habitId:{type:"string"},habitName:{type:"string"},name:{type:"string",minLength:1,maxLength:120},frequency:{type:"string",maxLength:40},reminder:{type:"string",pattern:timePattern}},required:["name"],additionalProperties:false}),execute:async(args:any)=>{const{state,habit}=await findHabit(client,userId,args.habitId,args.habitName);const name=String(args.name).trim();if(state.habits.some((i:any)=>i.id!==habit.id&&String(i.name||"").trim().toLowerCase()===name.toLowerCase()))throw new Error(`A habit named “${name}” already exists.`);const updated={...habit,name,...(args.frequency!==undefined?{frequency:String(args.frequency).trim()}:{}),...(args.reminder!==undefined?{reminder:args.reminder}:{})};await writeState(client,{...state,habits:state.habits.map((i:any)=>i.id===habit.id?updated:i)});const verified=await findHabit(client,userId,habit.id);if(verified.habit.name!==name)throw new Error("Habit update could not be verified.");return{changed:true,habit:verified.habit,verified:true};}});
  tools.delete_habit = tool({description:"Delete a habit only after an explicit user request.",inputSchema:jsonSchema({type:"object",properties:{habitId:{type:"string"},habitName:{type:"string"}},additionalProperties:false}),execute:async(args:any)=>{const{state,habit}=await findHabit(client,userId,args.habitId,args.habitName);await writeState(client,{...state,habits:state.habits.filter((i:any)=>i.id!==habit.id)});const after=await readState(client,userId);if(after.habits.some((i:any)=>i.id===habit.id))throw new Error("Habit deletion could not be verified.");return{deleted:true,habitId:habit.id,name:habit.name,verified:true};}});
  tools.set_reminder = tool({description:"Set a daily reminder time for an existing habit. Only use when explicitly requested.",inputSchema:jsonSchema({type:"object",properties:{habitId:{type:"string"},habitName:{type:"string"},time:{type:"string",pattern:timePattern}},required:["time"],additionalProperties:false}),execute:async(args:any)=>{const{state,habit}=await findHabit(client,userId,args.habitId,args.habitName);const updated={...habit,reminder:args.time};await writeState(client,{...state,habits:state.habits.map((i:any)=>i.id===habit.id?updated:i)});const verified=await findHabit(client,userId,habit.id);if(verified.habit.reminder!==args.time)throw new Error("Reminder update could not be verified.");return{changed:true,habit:verified.habit,reminder:args.time,verified:true};}});
  tools.log_outstand = tool({description:"Add a short item to the user's OUTSTAND log. Use only when explicitly requested.",inputSchema:jsonSchema({type:"object",properties:{text:{type:"string",minLength:1,maxLength:500},category:{type:"string",maxLength:60}},required:["text"],additionalProperties:false}),execute:async(args:any)=>{const state=await readState(client,userId);const item={id:crypto.randomUUID(),text:String(args.text).trim(),category:String(args.category||"general").trim()||"general",createdAt:new Date().toISOString()};await writeState(client,{...state,outstand:[item,...state.outstand].slice(0,500)});return{created:true,item,verified:true};}});
