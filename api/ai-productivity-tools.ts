...
export function createProductivityTools(client: Db, userId: string, accessToken: string): ToolSet {
  return {
    ...
    get_progress: tool({
      ...
      inputSchema: jsonSchema<{ roadmapId?: string }>({ type: "object", properties: { roadmapId: { type: "string", description: "Optional existing roadmap id" } }, additionalProperties: false }),
    ...
    set_task_progress: tool({
      description: "Update an existing roadmap task to pending, in_progress, completed, or skipped. Never invent a task. Never reopen a completed task unless the user explicitly asks to undo/reopen it.",
      ...
      execute: async ({ taskId, status }) => {
        ...
        if (error) throw error;
        return { changed: true, taskId, title: task.title, status };
      },
    }),
    ...
    create_roadmap: tool({
      ...
      execute: async ({ category, goal, durationDays, answers }) => {
        ...
        return { created: true, roadmapId: result.roadmapId, message: "Roadmap created successfully.", plan: result.plan ? { title: result.plan.title, durationDays: result.plan.durationDays, today: result.plan.today } : undefined };
      },
    }),
