...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  console.log("[Push] authenticated user", userId);
  ...
  console.log("[Push] subscription lookup", { userId, count: subs?.length ?? 0 });

  if (!subs?.length) {
    return send(res, 404, { error: "No push subscription found for this account", userId });
  }
