...
function systemPrompt(appContext: unknown, databaseContext: Awaited<ReturnType<typeof buildAiContext>>) {
  const ctx = appContext && typeof appContext === "object" ? appContext as Record<string, unknown> : {};
  const profile = databaseContext.profile as Record<string, unknown> | null;
  const roadmap = databaseContext.roadmap as Record<string, unknown> | null;
  const name = typeof profile?.display_name === "string" ? profile.display_name : typeof ctx.name === "string" ? ctx.name : "friend";
  const xp = Number(profile?.total_xp ?? ctx.xp ?? 0) || 0;
  const streak = Number(profile?.current_streak ?? 0) || 0;
  const bestStreak = Number(profile?.best_streak ?? ctx.bestStreak ?? 0) || 0;
  const tasks = databaseContext.todayTasks.slice(0, 12).map((task: any) => `${task.start_time || ""}-${task.end_time || ""} ${task.title} ${task.completed ? "done" : ""}`.trim()).join(" | ");
  const focus = databaseContext.recentFocus.slice(0, 8).map((item: any) => `${item.duration_minutes || 0}m ${item.task || "focus"}`).join(", ");
  const recentXp = databaseContext.recentXp.slice(0, 8).map((item: any) => `${item.amount} XP ${item.source}`).join(", ");

  return `You are OUTSTAND Intelligence, the user's personal productivity and learning guide.
User: ${name}
Authoritative XP: ${xp}
Current streak: ${streak}
Best streak: ${bestStreak}
Roadmap: ${roadmap ? `${roadmap.title || "Untitled"} — ${roadmap.goal || ""} — ${roadmap.status || ""} — target ${roadmap.target_date || ""}` : "No active roadmap"}
Today's roadmap tasks: ${tasks || "none found"}
Recent focus: ${focus || "none"}
Recent XP activity: ${recentXp || "none"}

Database context is authoritative for persistent user state. Client app context is supplementary and may be stale. Do not invent missing roadmap tasks, XP, streaks, or personal facts. If a roadmap exists, use its tasks and timing to give concrete next steps. If the user asks what to do now, prefer the next incomplete roadmap task when available. Keep answers practical and concise.

Formatting rules: never use markdown bold markers, never use asterisks for emphasis, never use parentheses. Prefer short paragraphs, hyphen bullets, and simple headings.`;
}
...
function getProvider(): ProviderChoice { const groqKey = env("GROQ_API_KEY"); const geminiKey = env("GEMINI_API_KEY", "GOOGLE_GENERATIVE_AI_API_KEY", "GOOGLE_API_KEY"); if (groqKey) return { name: "groq", model: createGroq({ apiKey: groqKey })("openai/gpt-oss-20b") }; if (geminiKey) return { name: "gemini", model: createGoogleGenerativeAI({ apiKey: geminiKey })("gemini-2.5-flash-lite") }; const error = new Error("No AI provider is configured on the server."); Object.assign(error, { status: 503, code: "AI_PROVIDER_CONFIG_MISSING" }); throw error; }
...
export const Route = createFileRoute("/api/chat")({ server: { handlers: {
  ...
  POST: async ({ request }) => {
    try {
      ...
      const databaseContext = await buildAiContext(auth.client, auth.userId);
      const modelMessages = uiMessages.slice(-10);
      const result = streamText({ model: provider.model, system: systemPrompt(parsed.data.appContext, databaseContext), messages: await convertToModelMessages(modelMessages), maxOutputTokens: 500, maxRetries: 0, abortSignal: request.signal,
        onError: (error) => { if (isQuotaError(error)) console.warn(`[AI] ${provider.name} quota/rate limit reached.`); else console.error(`[AI] ${provider.name} stream error:`, error); },
      });
      return result.toUIMessageStreamResponse({
        ...
        onFinish: async ({ responseMessage, isAborted }) => {
          ...
          try {
            ...
            if (assistantText) { const saved = await auth.client.from("chat_messages").insert({ conversation_id: conversationId, user_id: auth.userId, role: "assistant", content: assistantText }); if (saved.error) throw saved.error; }
