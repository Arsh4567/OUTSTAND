...
function MomentumPage() {
  ...
  const analyzeWeek = async () => {
    ...
    const response = await fetch("/api/dopamine-analysis", {
      ...
      body: JSON.stringify({ logs: weeklyLogs.filter((item) => item.recorded).map((item) => ({ log_date: item.log_date, score: item.score, positives: item.positives, negatives: item.negatives })) }),
    });
  ...
  if (loading) return <div className="flex min-h-screen items-center justify-center bg-[#02040a] px-6 text-center text-xs font-black uppercase tracking-[0.3em] text-cyan-300">Calibrating your momentum state...</div>;
