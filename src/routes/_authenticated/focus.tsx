...
import { BrainCircuit, Check, CheckCircle2, ChevronRight, ListChecks, Pause, Play, RotateCcw, Sparkles, Timer, Trash2 } from "lucide-react";
...
function FocusPage() {
    ...
    intervalRef.current = window.setInterval(() => {
      setSeconds((current) => {
        ...
        setMinutes((currentMinutes) => {
          ...
          setRunning(false);
          if (mode === "focus") {
            recordSession(Math.max(1, Math.round((Number(taskMinutes(task, queue)) || FOCUS_MINUTES))), true);
            ...
            setMode("break"); setMinutes(BREAK_MINUTES);
          } else {
            toast.success("Break complete", { description: queue.some((item) => !item.completed) ? "Your next task is ready." : "Ready for another focus session." });
            setMode("focus"); setMinutes(queue.find((item) => !item.completed)?.minutes ?? FOCUS_MINUTES);
          }
          return 0;
        });
        return 59;
  ...
  }, [running, mode, recordSession, task, queue]);

  useEffect(() => () => { if (intervalRef.current) window.clearInterval(intervalRef.current); }, []);

  const reset = () => { setRunning(false); setMinutes(mode === "focus" ? (queue.find((item) => !item.completed)?.minutes ?? FOCUS_MINUTES) : BREAK_MINUTES); setSeconds(0); };
  const toggle = () => setRunning((value) => !value);
  const startQueueItem = (item: QueueItem) => { if (running) return; setMode("focus"); setTask(item.title); setMinutes(item.minutes); setSeconds(0); setRunning(true); };
  ...
  const createBreakdown = async () => {
    ...
    try {
      ...
      setQueue((items) => [...items.filter((item) => !item.completed), ...newTasks]);
      setMilestone("");
  ...
  const totalSeconds = mode === "focus" ? Math.max(1, minutes * 60 + seconds + (running ? 0 : 0)) : BREAK_MINUTES * 60;
  const remainingSeconds = minutes * 60 + seconds;
  const initialSeconds = mode === "focus" ? Math.max(1, (queue.find((item) => item.title === task && !item.completed)?.minutes ?? FOCUS_MINUTES) * 60) : BREAK_MINUTES * 60;
  const progress = Math.max(0, Math.min(1, 1 - remainingSeconds / initialSeconds));
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_0.72fr]">
          <section className="overflow-hidden rounded-[2rem] border border-white/[0.08] bg-white/[0.035] shadow-2xl backdrop-blur-xl">
            <div className="border-b border-white/[0.06] p-5 sm:p-7"><div className="flex gap-2"><button type="button" onClick={() => { if (!running) { setMode("focus"); setMinutes(queue.find((item) => !item.completed)?.minutes ?? FOCUS_MINUTES); setSeconds(0); } }} className={`rounded-xl px-4 py-2 text-xs font-black ${mode === "focus" ? "bg-cyan-300 text-slate-950" : "text-slate-500 hover:bg-white/5 hover:text-white"}`}>Focus</button><button type="button" onClick={() => { if (!running) { setMode("break"); setMinutes(BREAK_MINUTES); setSeconds(0); } }} className={`rounded-xl px-4 py-2 text-xs font-black ${mode === "break" ? "bg-white text-slate-950" : "text-slate-500 hover:bg-white/5 hover:text-white"}`}>Break · 5m</button></div><label className="mt-5 block"><span className="text-[10px] font-black uppercase tracking-[.18em] text-slate-600">What are you working on?</span><input value={task} onChange={(e) => setTask(e.target.value)} disabled={running} maxLength={160} placeholder="e.g. Finish chemistry notes" className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm font-semibold text-white outline-none placeholder:text-slate-700 focus:border-cyan-300/40 disabled:opacity-60" /></label></div>
