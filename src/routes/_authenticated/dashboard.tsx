...
function DashboardHQ() {
  ...
  const loadDashboard = async (signal?: AbortSignal) => {
    ...
    const [statsRes, habitsRes] = await Promise.all([
      supabase.from("user_stats").select("total_xp, level, streak_days, current_level_xp, next_level_xp").eq("user_id", session.user.id).maybeSingle(),
  ...
  useEffect(() => {
    ...
    const refreshStats = async () => {
      const { data, error } = await supabase.from("user_stats").select("total_xp, level, streak_days, current_level_xp, next_level_xp").eq("user_id", userId).maybeSingle();
