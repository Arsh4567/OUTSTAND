...
import { Loader2, RefreshCw, ArrowRight, Target, Timer, CheckCircle2, SlidersHorizontal } from "lucide-react";
...
function DashboardPage() {
  ...
  const fallbackAction = !hasHabits
    ? { label: "Choose your habits", href: "/habits", icon: <SlidersHorizontal className="h-4 w-4" />, text: "Pick the habits that actually matter to you. You can choose up to 7, or start with none." }
    : nextHabit
  ...
  return <MotionConfig reducedMotion="user"><div className="outstand-dashboard min-h-screen overflow-x-hidden bg-background text-foreground"><main className="mx-auto w-full max-w-7xl space-y-4 px-3 pb-20 pt-4 sm:space-y-5 sm:px-6 sm:pt-6 lg:px-8 lg:pt-7">
    ...
    <section className="rounded-[22px] border border-primary/15 bg-primary/[0.035] p-4 shadow-sm sm:rounded-[24px] sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="text-[9px] font-black uppercase tracking-[0.2em] text-primary">Right now</p><h2 className="mt-1.5 break-words text-xl font-black tracking-tight text-foreground">{fallbackAction.label}</h2><p className="mt-1.5 max-w-2xl text-sm leading-6 text-muted-foreground">{fallbackAction.text}</p></div><Link to={fallbackAction.href as any} className="inline-flex min-h-11 w-full shrink-0 items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-[10px] font-black uppercase tracking-[0.14em] text-primary-foreground transition hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/70">{fallbackAction.icon}{fallbackAction.href === "/dashboard" ? "Go to habits" : fallbackAction.label}<ArrowRight className="h-3.5 w-3.5" /></Link></div>{scoreValue && <p className="mt-4 border-t border-border pt-4 text-[10px] font-bold uppercase tracking-[0.14em] text-muted-foreground">Latest daily score · <span className="text-foreground">{scoreValue}</span></p>}</section>
