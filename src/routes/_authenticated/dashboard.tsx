...
function DashboardPage() {
  ...
  if (isLoading) {
    return <div className="grid min-h-screen place-items-center bg-[#05070d] text-white"><div className="flex flex-col items-center gap-4"><div className="grid h-14 w-14 place-items-center rounded-2xl border border-cyan-300/10 bg-cyan-300/[0.05]"><Loader2 className="h-6 w-6 animate-spin text-cyan-300" /></div><p className="text-[10px] font-bold uppercase tracking-[0.28em] text-slate-500">Building your day</p></div></div>;
  }
  ...
  const completedHabits = habits.filter((habit) => habit.history?.includes(today.toISOString().slice(0, 10))).length;
  ...
  return <MotionConfig reducedMotion="user">
    <div className="relative min-h-screen overflow-x-hidden bg-[#05070d] text-slate-100">
      ...
      <main className="relative z-10 mx-auto max-w-7xl space-y-5 px-4 pb-20 pt-6 sm:px-6 lg:px-8 lg:pt-9">
