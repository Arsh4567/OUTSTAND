...
import { Activity, ArrowUpRight, CheckCircle2, Clock3, Flame, Focus, Loader2, Quote, Sparkles, Target, Trophy, Zap, ShieldCheck, Compass } from "lucide-react";
...
function DashboardHQ() {
  ...
  const loadDashboard = async () => {
    ...
    if (habitsRes.data) {
      const mapped: DailyQuest[] = habitsRes.data.map((row) => {
  ...
  useEffect(() => {
    let mounted = true;
    loadDashboard().catch((error) => console.error("Dashboard load failed", error)).finally(() => mounted && setIsLoading(false));
  ...
  const missionSource: DailyQuest[] = habits.length > 0 ? habits : fallbackMissions.map((mission) => ({ id: mission.id, completed: fallbackCompleted.has(mission.id), quest: mission as any }));
  ...
  return <div className="relative min-h-screen overflow-x-hidden bg-[#050816] text-slate-100">
