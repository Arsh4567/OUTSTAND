...
import { Bell, BellOff, Check, Clock3, Megaphone, Target, Zap, RefreshCw } from "lucide-react";
...
import { disablePushNotifications, getNotificationHistory, getNotificationPreferences, requestPushPermission, saveNotificationPreferences, type NotificationPreferences } from "@/lib/notification-engine";

export const Route = createFileRoute("/_authenticated/notifications")({ component: NotificationsPage });
const categories = [
  { key: "coaching_enabled", title: "Contextual guidance", description: "Useful nudges based on your actual habits, focus and progress.", icon: Zap },
  ...
  { key: "goals_enabled", title: "Goal reminders", description: "Keep important goals visible without constant interruptions.", icon: Target },
  { key: "motivational_enabled", title: "Momentum messages", description: "Occasional encouragement when it genuinely helps.", icon: Bell },
  { key: "updates_enabled", title: "OUTSTAND updates", description: "New features, improvements and important product news.", icon: Megaphone },
] as const;
...
function NotificationsPage() {
  ...
  const load = async () => {
    setLoadError(null);
    try { const [prefs, events] = await Promise.all([getNotificationPreferences(), getNotificationHistory()]); setPreferences(prefs); setHistory(events as NotificationEvent[]); }
    catch (error) { setLoadError(error instanceof Error ? error.message : "Could not load notification settings."); }
  };
  useEffect(() => { void load(); }, []);
  ...
  const enable = async () => { setBusy(true); try { await requestPushPermission(); setPreferences(await getNotificationPreferences()); toast.success("Notifications enabled."); } catch (error) { toast.error(error instanceof Error ? error.message : "Could not enable notifications."); } finally { setBusy(false); } };
  ...
  return <main className="mx-auto w-full max-w-5xl space-y-6 px-4 py-8 md:px-8">
    <section className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.025] p-6 md:p-8"><div className="relative z-10 flex flex-col gap-6 md:flex-row md:items-center md:justify-between"><div><div className="mb-3 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-xs font-medium text-slate-300"><Bell className="size-3.5" /> Notifications</div><h1 className="text-3xl font-semibold tracking-tight text-white md:text-4xl">Useful reminders. Nothing noisy.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-white/55">Choose exactly what deserves a place in your day. Set your preferences first, then turn push notifications on when you're ready.</p></div><Button size="lg" disabled={busy} onClick={() => void (preferences.push_enabled ? disable() : enable())} className="shrink-0">{preferences.push_enabled ? <><BellOff className="mr-2 size-4" />Pause notifications</> : <><Bell className="mr-2 size-4" />Enable notifications</>}</Button></div></section>
