...
export function OutstandPage() {
  ...
  const progress = challenge && missionMinutes > 0
    ? Math.min(100, Math.max(0, ((missionMinutes * 60 - (mins * 60 + secs)) / (missionMinutes * 60)) * 100))
    : 0;
        ...
        <main className="relative z-20 mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-7xl flex-col px-4 pb-10 pt-6 sm:px-6 lg:px-8 lg:pt-8">
          <header className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease }}>
              <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-cyan-400/15 bg-cyan-400/[0.06] px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.28em] text-cyan-200/80">
                <Sparkles className="h-3.5 w-3.5" /> Outstand protocol
              </div>
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-3">
              ...
              <Metric icon={<Flame />} value={challenge ? `${challenge.xpReward}` : "—"} label="xp ready" />
            </div>
          </header>
          ...
          <div className="mt-7 flex-1 rounded-[2rem] border border-white/[0.08] bg-white/[0.025] p-2 shadow-[0_30px_100px_rgba(0,0,0,0.35)] backdrop-blur-xl sm:p-3 lg:mt-8">
            <div className="relative flex min-h-[620px] items-center justify-center overflow-hidden rounded-[1.6rem] border border-white/[0.05] bg-black/20 px-1 py-8 sm:px-5 lg:min-h-[680px]">
              {challenge && !isShuffling && (
                <div className="pointer-events-none absolute inset-x-6 top-5 z-30 flex items-center justify-between text-[9px] font-bold uppercase tracking-[0.22em] text-slate-500 sm:inset-x-8">
                  <span className="flex items-center gap-2"><Activity className="h-3 w-3 text-cyan-300" /> Mission active</span>
                  <span>{Math.round(progress)}% complete</span>
                </div>
              )}
              {!challenge && !isShuffling && (
                <div className="pointer-events-none absolute bottom-6 left-1/2 z-10 flex -translate-x-1/2 items-center gap-2 whitespace-nowrap rounded-full border border-white/5 bg-black/30 px-4 py-2 text-[9px] font-bold uppercase tracking-[0.2em] text-slate-600 backdrop-blur-md">
                  <ArrowUpRight className="h-3 w-3" /> Tap the core to generate a mission
                </div>
              )}
              <FocusEngine
                challenge={challenge}
...
function Metric({ icon, value, label }: { icon: React.ReactNode; value: string; label: string }) {
  return (
    <div className="min-w-[78px] rounded-2xl border border-white/[0.07] bg-white/[0.035] px-3 py-3 text-center backdrop-blur-md sm:min-w-[92px]">
      <div className="mx-auto mb-1.5 flex w-fit text-cyan-300/70">{icon}</div>
...
function WifiStatus() {
  return <span className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,0.8)]" /> System online</span>;
}
