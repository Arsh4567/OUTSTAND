...
import { Camera, Check, Copy, Edit3, Flame, Save, Target, Timer, Trophy, TrendingUp, UserRound, Zap, X } from "lucide-react";
    ...
    <motion.main initial="hidden" animate="show" variants={{ hidden: {}, show: { transition: { staggerChildren: 0.045 } } }} className="mx-auto max-w-7xl space-y-5 px-4 pb-20 pt-3 sm:px-6 lg:space-y-7 lg:px-8">
      ...
      <div className="grid gap-5 lg:grid-cols-[.8fr_1.2fr]">
        <motion.section variants={{ hidden: { opacity: 0, y: 18 }, show: { opacity: 1, y: 0, transition: { duration: 0.65, ease } } }} className={cn(card, "p-6 sm:p-8")} onMouseMove={onMove} onMouseLeave={() => { x.set(0); y.set(0); }}>
          <motion.div style={{ rotateX, rotateY, transformStyle: "preserve-3d" }} className="flex min-h-[300px] flex-col justify-between">
            <div><div className="text-[10px] font-black uppercase tracking-[.25em] text-indigo-300/70">Current level</div><div className="mt-2 text-7xl font-black tracking-tighter text-white">{state.level}</div><p className="text-sm text-slate-500">{state.getRankTitle(state.level)} class</p></div>
            <div style={{ transform: "translateZ(28px)" }}><div className="mb-3 flex items-center justify-between text-xs font-bold text-slate-500"><span>Progress</span><span className="text-white">{state.into} / {state.need}</span></div><div className="h-3 overflow-hidden rounded-full border border-white/10 bg-black/40"><motion.div initial={{ width: 0 }} animate={{ width: `${state.pct}%` }} transition={{ duration: .9, ease }} className="h-full rounded-full bg-gradient-to-r from-indigo-500 via-cyan-400 to-emerald-300" /></div><p className="mt-3 text-xs text-slate-600">{Math.max(0, state.need - state.into)} XP to next level.</p></div>
          </motion.div>
        </motion.section>

        <section className={cn(card, "p-6 sm:p-8")}>
          <div className="flex items-end justify-between gap-4"><div><div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[.24em] text-cyan-300/80"><TrendingUp className="h-4 w-4" /> Momentum</div><h2 className="mt-2 text-2xl font-black text-white">Your recent signal</h2></div><div className="text-right"><div className="text-3xl font-black text-white">{state.stats.avg}</div><div className="text-[10px] font-black uppercase tracking-[.2em] text-slate-600">Avg score</div></div></div>
          <div className="mt-6 h-52 overflow-hidden rounded-2xl border border-white/[0.05] bg-black/10 p-2"><SimpleTrend logs={state.logs} /></div>
        </section>
      </div>
