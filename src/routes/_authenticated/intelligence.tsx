...
function IntelligencePage() {
  ...
  return <MotionConfig reducedMotion="user"><div className="relative min-h-[calc(100vh-72px)] overflow-hidden bg-[#02050b] text-white"><main className="relative z-10 mx-auto w-full max-w-[1400px] px-3 pb-8 pt-4 sm:px-5 lg:px-8 lg:pt-6">
    <motion.header initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .45, ease }} className="mb-5 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><div className="min-w-0"><div className="mb-2 flex items-center gap-2 text-[10px] font-black uppercase tracking-[.24em] text-slate-500"><span className="h-px w-6 bg-slate-700" />WORKSPACE</div><h1 className="max-w-3xl text-3xl font-black tracking-[-.04em] sm:text-4xl lg:text-[2.65rem]">Plan the work.<br/><span className="text-slate-400">Then get to it.</span></h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">Use your activity, habits and progress to decide the next useful action.</p></div><div className="grid w-full grid-cols-2 gap-2 sm:grid-cols-4 xl:w-auto"><ContextBadge icon={<CheckCircle2 />} label="Habits" value={`${completed}/${habitCount || 0}`} /><ContextBadge icon={<Zap />} label="XP" value={xp.toLocaleString()} /><ContextBadge icon={<Flame />} label="Streak" value={`${bestStreak}d`} /><ContextBadge icon={<Activity />} label="Focus" value={`${focusMinutes}m`} /></div></motion.header>
    <div className="grid gap-4 xl:grid-cols-[230px_minmax(0,1fr)_240px]">
      <aside className="hidden space-y-4 xl:block"><section className="rounded-[1.5rem] border border-white/[.08] bg-white/[.025] p-4"><div className="flex items-center gap-3"><OutstandRobotAvatar size="md"/><div><p className="text-sm font-black">OUTSTAND</p><p className="text-xs text-slate-500">Personal workspace</p></div></div><p className="mt-4 text-xs leading-5 text-slate-500">Your context stays inside the product. Use this space to turn it into action.</p><div className="mt-4 grid grid-cols-2 gap-2"><MiniStat icon={<Target />} value={`${completion}%`} label="habits today"/><MiniStat icon={<Activity />} value={String(sessionCount)} label="sessions"/><MiniStat icon={<Zap />} value={xp.toLocaleString()} label="xp"/><MiniStat icon={<Flame />} value={`${bestStreak}d`} label="streak"/></div></section><section className="rounded-[1.5rem] border border-white/[.07] bg-white/[.02] p-4"><div className="text-xs font-black text-white">Useful starting points</div><div className="mt-3 grid gap-2 text-xs text-slate-500">{['Plan the next few hours','Fix a habit that keeps slipping','Choose your highest-impact task','Review your momentum'].map((item)=><div key={item} className="rounded-xl border border-white/[.05] bg-white/[.025] px-3 py-2.5 transition hover:border-white/[.12] hover:text-slate-300">{item}</div>)}</div></section></aside>
      <section className="min-h-[min(760px,calc(100vh-205px))] overflow-hidden rounded-[1.6rem] border border-white/[.09] bg-[#040912]/95 shadow-[0_35px_100px_rgba(0,0,0,.4)]"><div className="flex items-center justify-between border-b border-white/[.07] bg-white/[.02] px-4 py-3 sm:px-5"><div className="flex min-w-0 items-center gap-3"><OutstandRobotAvatar size="sm"/><div className="min-w-0"><p className="truncate text-sm font-black">Next action</p><p className="truncate text-[10px] text-slate-600">Context synced for {firstName}</p></div></div><div className="flex items-center gap-2 text-[9px] font-bold uppercase tracking-[.14em] text-slate-600"><span className="h-1.5 w-1.5 rounded-full bg-emerald-400" />Available</div></div><div className="h-[calc(100%-55px)] min-h-0"><OutstandChatPanel initialMessages={messages} context={context} onClose={() => undefined} onClear={() => { setMessages([]); setHistoryKey((value) => value + 1); }} historyLoading={historyLoading} /></div></section>
      <aside className="space-y-4"><section className="rounded-[1.5rem] border border-white/[.07] bg-white/[.02] p-4"><div className="flex items-center justify-between"><div className="flex items-center gap-2 text-xs font-black"><Gauge className="h-4 w-4 text-slate-400"/> Today</div><span className="text-[9px] font-black uppercase tracking-[.16em] text-slate-600">Current</span></div><div className="mt-4 space-y-3"><ContextRow label="Habits completed" value={`${completed}/${habitCount || 0}`} progress={completion}/><ContextRow label="Daily score" value={String(context.dopamineScore)} progress={Math.max(0,Math.min(100,context.dopamineScore))}/><ContextRow label="XP this cycle" value={String(xp)} progress={Math.min(100,xp % 100)}/></div></section><section className="rounded-[1.5rem] border border-white/[.07] bg-white/[.02] p-4"><div className="text-xs font-black">One thing at a time</div><p className="mt-2 text-sm leading-6 text-slate-400">Good plans are useful only when they lead to a concrete next step.</p></section><section className="rounded-[1.5rem] border border-white/[.06] bg-black/20 p-4 text-center"><OutstandRobotAvatar size="md"/><div className="mt-3 flex items-center justify-center gap-2 text-[10px] font-bold text-emerald-200/70"><CheckCircle2 className="h-3.5 w-3.5"/> Context synced</div></section></aside>
    </div></main><QuickActions /></div></MotionConfig>;
}
