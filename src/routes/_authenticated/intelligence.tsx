...
import { Activity, Brain, CheckCircle2, Flame, Gauge, MessageCircle, Sparkles, Target, Zap } from "lucide-react";
      ...
      <div className="relative min-h-[calc(100vh-72px)] overflow-hidden bg-[#030713] text-white">
        ...
        <main className="relative z-10 mx-auto w-full max-w-[1500px] px-3 pb-6 pt-4 sm:px-5 lg:px-7 lg:pb-8 lg:pt-6">
          ...
          <div className="grid gap-4 xl:grid-cols-[260px_minmax(0,1fr)_270px]">
            <aside className="hidden space-y-4 xl:block">
              <section className="relative overflow-hidden rounded-[1.75rem] border border-cyan-300/10 bg-white/[0.025] p-4 shadow-[0_30px_90px_rgba(0,0,0,.28)] backdrop-blur-xl">
                ...
                <div className="mt-5 grid grid-cols-2 gap-2">
                  <MiniStat icon={<Zap />} value={String(context.xp)} label="XP" />
                  <MiniStat icon={<Flame />} value={String(context.bestStreak)} label="best streak" />
                  <MiniStat icon={<Target />} value={`${completion}%`} label="habits today" />
                  <MiniStat icon={<Activity />} value={String(sessionCount)} label="sessions" />
                </div>
              </section>
              <section className="rounded-[1.75rem] border border-white/[0.07] bg-white/[0.02] p-4">
                <div className="flex items-center gap-2 text-xs font-black text-white"><Brain className="h-4 w-4 text-cyan-300" /> What I can help with</div>
                <div className="mt-3 space-y-2 text-xs text-slate-500">
                  {['Plan the next few hours', 'Fix a habit that keeps slipping', 'Choose your highest-impact task', 'Review your momentum'].map((item) => <div key={item} className="rounded-xl border border-white/[0.05] bg-white/[0.025] px-3 py-2.5">{item}</div>)}
                </div>
              </section>
            </aside>
            ...
            <aside className="hidden space-y-4 xl:block">
              <section className="rounded-[1.75rem] border border-white/[0.07] bg-white/[0.02] p-4">
                <div className="flex items-center justify-between"><div className="flex items-center gap-2 text-xs font-black"><Gauge className="h-4 w-4 text-cyan-300" /> Today</div><span className="text-[9px] font-black uppercase tracking-[.18em] text-slate-600">Live context</span></div>
                <div className="mt-4 space-y-3">
                  <ContextRow label="Habits completed" value={`${completed}/${habitCount || 0}`} progress={completion} />
                  <ContextRow label="Dopamine score" value={String(context.dopamineScore)} progress={Math.max(0, Math.min(100, context.dopamineScore))} />
                  <ContextRow label="XP" value={String(context.xp)} progress={Math.min(100, context.xp % 100)} />
                </div>
              </section>
              <section className="overflow-hidden rounded-[1.75rem] border border-blue-300/10 bg-gradient-to-br from-blue-500/[0.08] via-cyan-400/[0.03] to-transparent p-4">
                <div className="flex items-center gap-2 text-xs font-black"><Sparkles className="h-4 w-4 text-cyan-300" /> Intelligence note</div>
                <p className="mt-3 text-sm leading-6 text-slate-300">Hey {firstName}. The useful question isn't “what should I do?” — it's “what is the smallest action that creates momentum right now?”</p>
              </section>
...
function MiniStat({ icon, value, label }: { icon: React.ReactNode; value: string; label: string }) {
  return <div className="rounded-xl border border-white/[0.06] bg-black/20 p-2.5"><div className="text-cyan-300/70">{icon}</div><div className="mt-1 text-sm font-black text-white">{value}</div><div className="text-[8px] font-black uppercase tracking-[.14em] text-slate-600">{label}</div></div>;
}

function ContextRow({ label, value, progress }: { label: string; value: string; progress: number }) {
