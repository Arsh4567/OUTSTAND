...
// 1. Mouse Spotlight Card (Tracks cursor for premium glow effect)
function SpotlightCard({ children, className = "" }: { children: React.ReactNode, className?: string }) {
  ...
  return (
    <motion.div
      className={`group relative overflow-hidden rounded-[2rem] border border-white/10 bg-[#0a0f1a]/80 shadow-2xl backdrop-blur-xl ${className}`}
...
// 2. Magnetic Button (Pulls towards mouse slightly)
...
function LandingPage() {
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-[#02040a] text-slate-100 font-sans overflow-x-hidden relative selection:bg-cyan-500/30">
      ...
      {/* Scroll Progress Bar */}
      <motion.div 
        className="fixed top-0 left-0 right-0 h-1 bg-gradient-to-r from-cyan-400 via-blue-500 to-indigo-500 z-[100] origin-left"
        style={{ scaleX: progressHeight }} 
      />

      {/* --- CINEMATIC BACKGROUND GLOWS (With complex movement) --- */}
      <motion.div style={{ scale: scaleBackground }} className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
        <motion.div 
          animate={{ x: [0, 50, 0], y: [0, -50, 0], scale: [1, 1.2, 1], opacity: [0.15, 0.3, 0.15] }} 
          transition={{ duration: 15, repeat: Infinity, ease: "linear" }}
          className="absolute top-[-10%] left-[-10%] w-[60vw] h-[60vw] rounded-full bg-blue-600/30 blur-[180px]" 
        />
        <motion.div 
          animate={{ x: [0, -60, 0], y: [0, 40, 0], scale: [1, 1.3, 1], opacity: [0.1, 0.25, 0.1] }} 
          transition={{ duration: 20, repeat: Infinity, ease: "linear", delay: 2 }}
          className="absolute top-[30%] right-[-20%] w-[50vw] h-[50vw] rounded-full bg-purple-600/20 blur-[150px]" 
        />
      </motion.div>

      {/* --- GLASS NAV --- */}
      <motion.div initial={{ y: -100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ type: "spring", stiffness: 100, damping: 20, delay: 0.5 }} className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-2rem)] max-w-5xl">
      ...
      <main className="relative z-10">
        ...
        {/* --- HERO SECTION --- */}
        <motion.div style={{ y: heroY, opacity: heroOpacity }} className="mx-auto max-w-6xl px-4 pt-48 pb-32 text-center flex flex-col items-center justify-center min-h-[95vh] perspective-[1000px]">
          
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1, ease: "easeOut" }} className="mb-8 inline-flex items-center gap-2 rounded-full border border-cyan-500/30 bg-cyan-500/10 px-4 py-2 text-xs sm:text-sm font-bold tracking-wide text-cyan-400 backdrop-blur-md shadow-[0_0_30px_rgba(6,182,212,0.2)]">
          ...
          <motion.h1 variants={staggerContainer} initial="hidden" animate="visible" className="text-6xl sm:text-7xl md:text-8xl lg:text-[6rem] font-black tracking-tighter text-white leading-[1.05] max-w-5xl flex flex-wrap justify-center gap-x-4">
            {words.map((word, i) => (
              <motion.span key={i} variants={wordAnimation} className={word === "Life." || word === "10." ? "text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-500 to-indigo-400 drop-shadow-[0_0_30px_rgba(59,130,246,0.5)]" : ""}>
                {word}
              </motion.span>
            ))}
          </motion.h1>

          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1, duration: 0.8 }} className="mx-auto mt-8 max-w-2xl text-base sm:text-lg md:text-xl text-slate-400 font-medium leading-relaxed">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.2, duration: 0.8 }} className="mt-12">
          ...
          {/* ABSTRACT FLOATING UI DASHBOARD (Replaces Images) */}
          <motion.div 
            initial={{ opacity: 0, y: 150, rotateX: 40 }} 
            animate={{ opacity: 1, y: 0, rotateX: 0 }} 
            transition={{ delay: 1.4, duration: 1.5, type: "spring", stiffness: 50, damping: 20 }}
            className="mt-28 relative w-full max-w-4xl mx-auto h-[400px] perspective-[2000px] pointer-events-none"
          >
            {/* Center Main Card */}
            <motion.div 
              animate={{ y: [-10, 10, -10], rotateZ: [-1, 1, -1] }} transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
              className="absolute left-1/2 top-0 -translate-x-1/2 w-full max-w-2xl bg-[#0a0f1a]/80 backdrop-blur-2xl border border-white/10 rounded-[2rem] p-6 shadow-[0_0_80px_rgba(59,130,246,0.2)] z-20"
            >
              ...
              <div className="space-y-3">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="h-16 bg-white/[0.02] border border-white/5 rounded-xl flex items-center px-4 gap-4 relative overflow-hidden">
                     <motion.div initial={{ x: "-100%" }} animate={{ x: "100%" }} transition={{ duration: 3, repeat: Infinity, delay: i * 0.5, ease: "linear" }} className="absolute inset-0 w-1/2 bg-gradient-to-r from-transparent via-white/[0.05] to-transparent skew-x-12" />
            ...
            <motion.div 
              animate={{ y: [10, -15, 10], rotateY: [0, 10, 0] }} transition={{ duration: 7, repeat: Infinity, ease: "easeInOut", delay: 1 }}
              className="absolute left-[5%] top-20 w-64 bg-gradient-to-br from-[#0a0f1a] to-[#050810] border border-purple-500/30 rounded-2xl p-5 shadow-2xl z-10 backdrop-blur-xl"
            >
              ...
              <div className="w-32 h-32 mx-auto rounded-full border-[8px] border-purple-500/20 flex items-center justify-center relative">
                <motion.div animate={{ rotate: 360 }} transition={{ duration: 10, repeat: Infinity, ease: "linear" }} className="absolute inset-[-8px] rounded-full border-t-[8px] border-purple-500" />
            ...
            <motion.div 
              animate={{ y: [-15, 15, -15], rotateY: [0, -10, 0] }} transition={{ duration: 8, repeat: Infinity, ease: "easeInOut", delay: 2 }}
              className="absolute right-[5%] top-10 w-72 bg-gradient-to-br from-[#0a0f1a] to-[#050810] border border-orange-500/30 rounded-2xl p-5 shadow-2xl z-30 backdrop-blur-xl"
            >
        ...
        <div className="max-w-6xl mx-auto px-4 py-32 relative z-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="md:col-span-2">
              <SpotlightCard className="p-10 h-full flex flex-col justify-between group/card">
                ...
                <div className="relative z-10 w-full md:w-2/3">
                  <div className="w-16 h-16 bg-cyan-500/10 border border-cyan-500/30 rounded-2xl flex items-center justify-center mb-8 shadow-[0_0_20px_rgba(6,182,212,0.2)]">
                ...
                {/* Abstract UI element instead of image */}
                <div className="absolute -bottom-12 -right-12 w-80 h-64 bg-[#050810] border border-cyan-500/20 rounded-2xl shadow-2xl rotate-[-5deg] group-hover/card:rotate-[-2deg] group-hover/card:translate-y-[-10px] transition-all duration-500 p-4">
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}>
              <SpotlightCard className="p-10 h-full">
                ...
                <div className="relative z-10">
                  <div className="w-16 h-16 bg-purple-500/10 border border-purple-500/30 rounded-2xl flex items-center justify-center mb-8 shadow-[0_0_20px_rgba(168,85,247,0.2)]">
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 }} className="md:col-span-3">
              <SpotlightCard className="p-10 md:p-14 flex flex-col md:flex-row items-center gap-10">
                ...
                <div className="relative z-10 md:w-1/2">
                  <div className="flex-shrink-0 w-20 h-20 bg-orange-500/10 border border-orange-500/30 rounded-2xl flex items-center justify-center mb-8 shadow-[0_0_30px_rgba(249,115,22,0.2)]">
    ...
    {/* Abstract Arena UI element instead of image */}
                 <div className="relative w-full md:w-1/2 h-64 mt-10 md:mt-0 perspective-[1000px]">
                    <motion.div 
                      animate={{ rotateY: [-5, 5, -5], rotateX: [5, -5, 5] }} 
                      transition={{ duration: 10, repeat: Infinity, ease: "linear" }}
                      className="absolute inset-0 bg-[#050810] border border-orange-500/30 rounded-[2rem] p-6 shadow-[0_0_50px_rgba(249,115,22,0.15)] flex flex-col justify-center items-center"
                    >
        ...
        {/* --- BOTTOM CTA --- */}
        <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: "-200px" }} transition={{ type: "spring", stiffness: 50 }} className="max-w-5xl mx-auto px-4 py-40 text-center relative z-20">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[150%] h-[500px] bg-gradient-to-r from-blue-600/20 via-cyan-500/10 to-purple-600/20 rounded-full blur-[150px] pointer-events-none" />
          
          <motion.div animate={{ rotate: 360 }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }} className="inline-block mb-8">
