  ...
  return (
    <motion.div whileHover={{ y: -2 }} whileTap={{ scale: 0.98 }} transition={{ type: "spring", stiffness: 350, damping: 24 }}>
      <Button
        ...
        className={
          secondary
            ? "h-13 rounded-full border border-white/10 bg-white/[0.045] px-7 text-white backdrop-blur-xl hover:bg-white/[0.09]"
            : "h-13 rounded-full bg-white px-8 text-[#05070d] shadow-[0_0_55px_rgba(125,240,255,0.28)] hover:bg-cyan-50"
        }
      >
        {children}
      </Button>
...
function FeatureCard({ icon: Icon, title, text }: { icon: typeof Target; title: string; text: string }) {
  return (
    <motion.article
      ...
      whileHover={{ y: -4 }}
      className="group rounded-[24px] border border-white/[0.08] bg-white/[0.035] p-5 backdrop-blur-xl transition-colors hover:border-cyan-300/20 hover:bg-white/[0.055]"
    >
      <div className="grid h-10 w-10 place-items-center rounded-xl border border-cyan-300/15 bg-cyan-300/10 text-cyan-200">
  ...
  return (
    ...
    >
      <div className="flex h-12 items-center gap-2 border-b border-white/[0.07] px-5">
        ...
        <span className="h-2.5 w-2.5 rounded-full bg-white/15" />
        <span className="h-2.5 w-2.5 rounded-full bg-white/15" />
        <span className="ml-3 text-[10px] font-medium tracking-[0.2em] text-slate-500">OUTSTAND / TODAY</span>
      </div>
      <div className="grid lg:grid-cols-[1.15fr_0.85fr]">
        <div className="p-6 sm:p-9">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="hidden rounded-2xl border border-cyan-300/15 bg-cyan-300/10 px-4 py-3 text-right sm:block">
              <div className="text-[9px] uppercase tracking-widest text-cyan-200/70">Momentum</div>
        ...
        <div className="border-t border-white/[0.07] bg-white/[0.018] p-6 lg:border-l lg:border-t-0 lg:p-9">
          <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-indigo-200">
            <BrainCircuit className="h-4 w-4" /> AI guidance
          </div>
          <div className="mt-5 rounded-3xl border border-indigo-300/10 bg-indigo-400/[0.06] p-5">
            <p className="text-sm leading-6 text-slate-300">Your recent focus is consistent. Keep today's session short and targeted, then review the weak concept before moving on.</p>
          </div>
          ...
          <div className="mt-6 grid grid-cols-2 gap-3">
            <div className="rounded-2xl border border-white/[0.07] bg-white/[0.025] p-4"><Flame className="h-4 w-4 text-orange-300" /><p className="mt-2 text-xs text-slate-500">Streak</p><p className="mt-1 font-black text-white">7 days</p></div>
            <div className="rounded-2xl border border-white/[0.07] bg-white/[0.025] p-4"><Bell className="h-4 w-4 text-cyan-300" /><p className="mt-2 text-xs text-slate-500">Alerts</p><p className="mt-1 font-black text-white">2 new</p></div>
          </div>
        </div>
      </div>
    </motion.div>
      ...
      <nav className="fixed left-1/2 top-4 z-50 w-[calc(100%-1.5rem)] max-w-6xl -translate-x-1/2 rounded-2xl border border-white/[0.09] bg-[#050810]/70 px-4 py-3 shadow-2xl backdrop-blur-2xl sm:px-6" aria-label="Main navigation">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden items-center gap-7 text-xs font-medium text-slate-400 md:flex">
            ...
            <a href="#social" className="transition hover:text-white">Social</a>
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => setMobileOpen((v) => !v)} aria-expanded={mobileOpen} aria-label="Toggle navigation" className="grid h-9 w-9 place-items-center rounded-full border border-white/10 bg-white/[0.04] text-slate-300 md:hidden">
              <ChevronDown className={`h-4 w-4 transition-transform ${mobileOpen ? "rotate-180" : ""}`} />
            </button>
          </div>
        </div>
        {mobileOpen && (
          <div className="mt-3 grid gap-1 border-t border-white/[0.07] pt-3 md:hidden">
            {["features", "how-it-works", "social", "cta"].map((id) => <a key={id} href={`#${id}`} onClick={closeMobile} className="rounded-xl px-3 py-3 text-sm font-semibold capitalize text-slate-300 hover:bg-white/[0.05] hover:text-white">{id.replaceAll("-", " ")}</a>)}
      ...
      <main>
        <section className="relative flex min-h-screen items-center px-5 pb-20 pt-32 sm:px-8">
          <motion.div style={{ y: heroY }} className="mx-auto w-full max-w-6xl text-center">
            <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="mx-auto mb-7 inline-flex items-center gap-2 rounded-full border border-cyan-200/15 bg-cyan-300/[0.06] px-4 py-2 text-[10px] font-bold uppercase tracking-[0.22em] text-cyan-200/90 backdrop-blur-xl"><Sparkles className="h-3.5 w-3.5" /> One system for your next level</motion.div>
            <motion.h1 initial="hidden" animate="visible" className="mx-auto max-w-5xl text-[clamp(3.4rem,8.5vw,8.4rem)] font-black leading-[0.88] tracking-[-0.07em]">
            ...
            <div className="mt-9 flex flex-wrap items-center justify-center gap-3">
              ...
              <CTA secondary><a href="#features" className="flex items-center gap-2"><CirclePlay className="h-4 w-4" /> Explore everything</a></CTA>
            </div>
            <div className="mx-auto mt-9 grid max-w-2xl grid-cols-2 gap-2 text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500 sm:grid-cols-4">
        ...
        <section id="how-it-works" className="relative px-5 py-24 sm:px-8 sm:py-32">
          <div className="mx-auto max-w-6xl">
            <div className="max-w-3xl">
              ...
              <h2 className="text-4xl font-black tracking-[-0.05em] sm:text-6xl">From “I should” to “I did.”</h2>
            ...
            <div className="mt-12 grid gap-4 md:grid-cols-4">
              {[
                ["01", "Tell the AI", "Give the assistant your goal, constraints and current situation."],
                ["02", "Get your roadmap", "Turn the goal into milestones, tasks and a clear next step."],
                ["03", "Do the work", "Use focus sessions, habits and daily actions to execute."],
                ["04", "See momentum", "XP, streaks, activity and AI feedback show what to improve next."],
        ...
        <section id="features" className="border-y border-white/[0.06] bg-white/[0.018] px-5 py-24 sm:px-8 sm:py-32">
          <div className="mx-auto max-w-6xl">
            <div className="max-w-3xl">
              <div className="mb-4 text-[10px] font-bold uppercase tracking-[0.25em] text-cyan-300/70">Everything in OUTSTAND</div>
              <h2 className="text-4xl font-black tracking-[-0.05em] sm:text-6xl">More than a to-do list.</h2>
        ...
        <section id="intelligence" className="px-5 py-24 sm:px-8 sm:py-32">
          <div className="mx-auto grid max-w-6xl items-center gap-12 lg:grid-cols-[0.85fr_1.15fr]">
            <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
              ...
              <h2 className="text-4xl font-black tracking-[-0.05em] sm:text-6xl">AI that knows the system around you.</h2>
              ...
              <div className="mt-7 space-y-3">
                {["Ask what to work on next", "Get help with your current roadmap", "Turn setbacks into a revised plan", "Use your real progress as context"].map((item) => <div key={item} className="flex items-center gap-3 text-sm font-semibold text-slate-200"><ShieldCheck className="h-4 w-4 text-cyan-300" />{item}</div>)}
              </div>
            </motion.div>
            <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="rounded-[30px] border border-white/10 bg-white/[0.035] p-5 shadow-2xl backdrop-blur-xl sm:p-7">
              <div className="flex items-center gap-3 border-b border-white/[0.07] pb-5"><div className="grid h-9 w-9 place-items-center rounded-xl bg-indigo-400/10 text-indigo-200"><BrainCircuit className="h-4 w-4" /></div><div><p className="text-sm font-bold text-white">OUTSTAND AI</p><p className="text-[10px] text-slate-500">Context-aware guidance</p></div></div>
        ...
        <section id="social" className="border-y border-white/[0.06] bg-white/[0.018] px-5 py-24 sm:px-8 sm:py-32">
          <div className="mx-auto max-w-6xl">
            <div className="max-w-3xl">
              <div className="mb-4 text-[10px] font-bold uppercase tracking-[0.25em] text-cyan-300/70">Don't grow alone</div>
              <h2 className="text-4xl font-black tracking-[-0.05em] sm:text-6xl">Your progress has a social side.</h2>
        ...
        <section id="momentum" className="px-5 py-24 sm:px-8 sm:py-32">
          <div className="mx-auto max-w-6xl rounded-[32px] border border-cyan-300/10 bg-gradient-to-br from-cyan-300/[0.07] via-white/[0.025] to-indigo-400/[0.07] p-7 sm:p-10 lg:p-14">
            <div className="grid gap-10 lg:grid-cols-[1fr_0.8fr] lg:items-center">
              <div>
                <div className="mb-4 flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.22em] text-cyan-300"><Zap className="h-3.5 w-3.5" /> Momentum engine</div>
                <h2 className="text-4xl font-black tracking-[-0.05em] sm:text-5xl">Small actions become visible progress.</h2>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[{ icon: Zap, label: "Focus", value: "2 XP / min" }, { icon: Flame, label: "Streaks", value: "Keep going" }, { icon: Trophy, label: "Levels", value: "Track growth" }, { icon: Bell, label: "Alerts", value: "Stay on time" }].map(({ icon: Icon, label, value }) => <div key={label} className="rounded-2xl border border-white/[0.08] bg-black/20 p-5"><Icon className="h-4 w-4 text-cyan-300" /><p className="mt-4 text-[10px] font-bold uppercase tracking-[0.16em] text-slate-600">{label}</p><p className="mt-1 text-sm font-black text-white">{value}</p></div>)}
              </div>
            </div>
          </div>
        </section>

        <section id="cta" className="relative px-5 pb-24 pt-10 sm:px-8 sm:pb-36">
          <div className="mx-auto max-w-4xl text-center">
            <div className="mx-auto grid h-12 w-12 place-items-center rounded-2xl border border-cyan-300/20 bg-cyan-300/10 text-cyan-200"><Sparkles className="h-5 w-5" /></div>
            <h2 className="mt-7 text-4xl font-black tracking-[-0.05em] sm:text-6xl">Stop collecting plans.<br /><span className="text-white/35">Start building momentum.</span></h2>
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-3"><CTA><Link to="/auth" className="flex items-center gap-2">Create my OUTSTAND system <ArrowRight className="h-4 w-4" /></Link></CTA></div>
