...
import { ArrowRight, CheckCircle2, Clock3, Pencil, Plus, Target } from "lucide-react";
...
function RoadmapPage() {
  ...
  const daysLeft = roadmap.roadmap?.target_date ? daysBetweenTodayAnd(roadmap.roadmap.target_date) : Math.max(0, Number(roadmap.roadmap?.duration_days || 0) - roadmap.todayIndex);
  ...
  return <main className="relative min-h-screen overflow-hidden bg-[#050812] px-4 pb-24 pt-4 text-slate-100 sm:px-6 lg:px-8">{ambient}<div className="relative z-10 mx-auto max-w-7xl space-y-5 sm:space-y-7"><section className="rounded-[2rem] border border-white/10 bg-[#07101d]/92 p-5 shadow-[0_24px_80px_-55px_rgba(34,211,238,.25)] backdrop-blur-xl sm:p-7"><div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0"><h1 className="text-3xl font-black tracking-tight sm:text-4xl">{currentTitle}</h1><div className="mt-4 flex flex-wrap items-center gap-2 text-[10px] font-black uppercase tracking-[.12em]"><span className="rounded-full border border-cyan-300/15 bg-cyan-300/[.05] px-3 py-1.5 text-cyan-100">{roadmap.roadmap.category || "goal"}</span><span className="rounded-full border border-white/10 bg-white/[.03] px-3 py-1.5 text-slate-500">Day {roadmap.todayIndex} of {roadmap.roadmap.duration_days}</span><span className="rounded-full border border-white/10 bg-white/[.03] px-3 py-1.5 text-slate-300">{formatDaysLeft(daysLeft)}</span></div></div><div className="flex flex-wrap gap-2"><div className="rounded-2xl border border-white/10 bg-white/[.03] px-4 py-3"><div className="text-[10px] font-black uppercase tracking-[.14em] text-slate-500">Overall</div><div className="mt-1 text-xl font-black">{overallProgress}%</div></div><button type="button" onClick={() => setEditOpen(true)} className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[.03] px-4 py-3 text-xs font-black text-slate-200 transition hover:bg-white/[.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-200"><Pencil className="h-4 w-4" /> Edit</button></div></div>

  <div className="mt-5"><button type="button" aria-expanded={showRoadmaps} onClick={() => setShowRoadmaps((value) => !value)} className="group flex w-full items-center justify-between rounded-2xl border border-white/10 bg-white/[.025] px-4 py-3 text-left transition hover:border-cyan-300/15 hover:bg-white/[.04]"><div><div className="text-[10px] font-black uppercase tracking-[.16em] text-slate-400">My roadmaps</div><div className="mt-1 text-xs text-slate-600">Choose a goal to continue.</div></div><div className="flex items-center gap-2"><span className="rounded-full border border-white/10 px-2.5 py-1 text-[10px] font-black uppercase tracking-[.12em] text-slate-500">{roadmap.roadmaps.length}/4</span><span className={`text-lg text-slate-500 transition-transform ${showRoadmaps ? "rotate-180" : ""}`}>⌄</span></div></button>{showRoadmaps && <div className="mt-2 rounded-2xl border border-white/10 bg-[#050d18]/70 p-3"><div className="grid gap-2 sm:grid-cols-2">{roadmap.roadmaps.map((item, index) => <button key={item.id} type="button" title={displayTitle(item.title, item.goal)} onClick={() => void roadmap.selectRoadmap(item.id)} className={`rounded-2xl border px-3.5 py-3 text-left transition ${item.id === roadmap.roadmap?.id ? "border-cyan-300/30 bg-cyan-300/[.08] shadow-[0_12px_35px_-25px_rgba(34,211,238,.8)]" : "border-white/10 bg-white/[.02] hover:border-white/15 hover:bg-white/[.05]"}`}><div className="flex items-center justify-between gap-2"><span className="text-[9px] font-black uppercase tracking-[.14em] text-slate-600">{index + 1}</span>{item.id === roadmap.roadmap?.id && <span className="h-1.5 w-1.5 rounded-full bg-cyan-200" />}</div><div className="mt-2 line-clamp-2 text-sm font-black leading-5 text-slate-200">{cleanRoadmapName(item.title, item.goal)}</div><div className="mt-1 text-[10px] font-semibold uppercase tracking-[.12em] text-slate-600">{item.category || "goal"}</div></button>)}<button type="button" disabled={roadmap.roadmaps.length >= 4} onClick={() => { roadmap.resetIntake(); roadmap.setAnswers({}); setCategory("skill_learning"); setShowOnboarding(true); setShowRoadmaps(false); }} className="rounded-2xl border border-dashed border-white/10 bg-white/[.01] px-3.5 py-3 text-left text-xs font-black text-slate-500 transition hover:border-cyan-300/20 hover:text-cyan-100 disabled:cursor-not-allowed disabled:opacity-40"><Plus className="mb-2 h-4 w-4" /> Add roadmap</button></div></div>}</div>
  </section>
