...
export async function requestPushPermission() { if (!("Notification" in window) || !("serviceWorker" in navigator) || !("PushManager" in window)) throw new Error("Push notifications aren't supported by this browser."); const permission = Notification.permission === "default" ? await Notification.requestPermission() : Notification.permission; if (permission !== "granted") throw new Error("Notification permission was not granted."); const { data: { session } } = await supabase.auth.getSession(); if (!session) throw new Error("Please sign in first."); const registration = await navigator.serviceWorker.register("/sw.js").then(() => navigator.serviceWorker.ready); const vapid = import.meta.env.VITE_VAPID_PUBLIC_KEY; if (!vapid) throw new Error("Push notifications are not configured yet."); const applicationServerKey = base64UrlToUint8Array(vapid); const existing = await registration.pushManager.getSubscription(); const subscription = existing ?? await registration.pushManager.subscribe({ userVisibleOnly: true, applicationServerKey }); const json = subscription.toJSON(); if (!json.endpoint || !json.keys?.auth || !json.keys?.p256dh) throw new Error("The browser returned an invalid push subscription."); const { error: subscriptionError } = await supabase.from("push_subscriptions").upsert({ user_id: session.user.id, endpoint: json.endpoint, auth_key: json.keys.auth, p256dh_key: json.keys.p256dh }, { onConflict: "user_id,endpoint" }); if (subscriptionError) { if (!existing) await subscription.unsubscribe().catch(() => undefined); throw new Error(`Could not save your push subscription: ${subscriptionError.message}`); } await saveNotificationPreferences({ push_enabled: true }); return subscription; }
