...
type Pending = { resolve: (value: EngineLine) => void; reject: (error: Error) => void };
...
function createWorker() {
  const source = `
    ...
    try {
      ...
      engine.addMessageListener ? engine.addMessageListener(handleLine) : engine.onmessage = handleLine;
...
function getWorker() {
  ...
  worker.addEventListener('message', (event: MessageEvent) => {
    ...
    pending.delete(id);
    request?.resolve({ score: null, mate: null, bestMove: event.data.bestMove || null });
  });
