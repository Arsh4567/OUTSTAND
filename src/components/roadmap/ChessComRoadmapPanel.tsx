...
type Props = { onLoaded?: (data: ChessData) => void };

const ratingLabel = (value: number | null) => value == null ? "—" : String(value);

export function ChessComRoadmapPanel({ onLoaded }: Props) {
  ...
  return <section className="rounded-[2rem] border border-orange-300/10 bg-white/[0.025] p-5 sm:p-7">
    <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
      ...
      {data && <button type="button" onClick={connect} disabled={loading} className="inline-flex shrink-0 items-center gap-2 rounded-xl border border-white/10 bg-white/[0.03] px-4 py-2.5 text-xs font-black text-slate-200 disabled:opacity-50"><RefreshCw className="h-4 w-4" />Refresh</button>}
    </div>
    <div className="mt-5 flex flex-col gap-2 sm:flex-row"><input value={username} onChange={(e) => setUsername(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") void connect(); }} placeholder="Chess.com username" autoComplete="off" className="min-w-0 flex-1 rounded-2xl border border-white/[0.08] bg-black/20 px-4 py-3 text-sm font-semibold text-white outline-none placeholder:text-slate-600 focus:border-orange-200/30" /><button type="button" onClick={() => void connect()} disabled={loading || !username.trim()} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-orange-200 px-5 py-3 text-sm font-black text-slate-950 transition hover:bg-orange-100 disabled:cursor-not-allowed disabled:opacity-50">{loading ? <><Loader2 className="h-4 w-4 animate-spin" />Loading Chess.com…</> : "Connect"}</button></div>
    ...
    {data && <div className="mt-5 rounded-[1.5rem] border border-white/[0.07] bg-black/10 p-4 sm:p-5"><div className="flex items-center gap-3"><div className="h-12 w-12 overflow-hidden rounded-full border border-white/10 bg-white/5">{data.profile.avatar ? <img src={data.profile.avatar} alt={`${data.profile.username} avatar`} className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center text-xs font-black text-slate-600">C</div>}</div><div><div className="flex items-center gap-2"><span className="text-base font-black text-white">{data.profile.username}</span>{data.profile.title && <span className="rounded-md border border-orange-200/15 bg-orange-200/[0.06] px-1.5 py-0.5 text-[9px] font-black text-orange-100">{data.profile.title}</span>}</div><div className="mt-1 text-[10px] font-bold uppercase tracking-[.14em] text-slate-600">Live public profile data</div></div></div><div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-4">{([["Rapid", data.ratings.rapid], ["Blitz", data.ratings.blitz], ["Bullet", data.ratings.bullet], ["Tactics", data.ratings.tactics]] as const).map(([label, value]) => <div key={label} className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-3"><div className="text-[9px] font-black uppercase tracking-[.15em] text-slate-600">{label}</div><div className="mt-1 text-xl font-black tabular-nums text-white">{ratingLabel(value)}</div></div>)}</div></div>}
  </section>;
}
