...
export function RoadmapEditDialog({ open, roadmap, onOpenChange, onSave, onSmartChange, onDelete, saving, askingAI, deleting }: Props) {
  ...
  useEffect(() => {
    if (!open) return;
    setTitle(roadmap?.title || ""); setGoal(roadmap?.goal || ""); setRequest(""); setConfirmDelete(false);
    const timer = window.setTimeout(() => titleRef.current?.focus(), 40); return () => window.clearTimeout(timer);
  }, [open, roadmap?.id, roadmap?.title, roadmap?.goal]);
  useEffect(() => {
    if (!open) return;
    const onKeyDown = (event: KeyboardEvent) => { if (event.key === "Escape" && !busy) onOpenChange(false); };
    window.addEventListener("keydown", onKeyDown); return () => window.removeEventListener("keydown", onKeyDown);
  }, [open, busy, onOpenChange]);
  if (!roadmap) return null;

  const cleanTitle = title.trim(); const cleanGoal = goal.trim(); const canSave = cleanTitle.length >= 2 && cleanGoal.length >= 5 && !busy;
  const applySmartChange = async () => { const next = request.trim(); if (next.length < 5 || busy || !onSmartChange) return; try { await onSmartChange(next); setRequest(""); } catch (error) { toast.error(error instanceof Error ? error.message : "Could not apply smart change."); } };
  const handleDelete = async () => { if (!onDelete || busy) return; try { await onDelete(); } catch (error) { toast.error(error instanceof Error ? error.message : "Could not delete roadmap."); } };

  return <AnimatePresence>{open && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[90] flex items-end justify-center bg-[#020617]/85 p-0 backdrop-blur-md sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="roadmap-edit-title" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) onOpenChange(false); }}>
    <motion.div initial={{ opacity: 0, y: 18, scale: .985 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 12, scale: .985 }} className="relative flex max-h-[92vh] w-full max-w-xl flex-col overflow-hidden rounded-t-[2rem] border border-cyan-300/10 bg-[#07101d] shadow-[0_40px_120px_-50px_rgba(34,211,238,.45)] sm:rounded-[2rem]" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="relative overflow-y-auto p-5 sm:p-7">
        <div className="flex items-start justify-between gap-4"><div><div className="inline-flex items-center gap-2 rounded-full border border-cyan-200/20 bg-cyan-200/10 px-3 py-1 text-[9px] font-black uppercase tracking-[.18em] text-cyan-100"><Pencil className="h-3 w-3" /> Edit roadmap</div><h2 id="roadmap-edit-title" className="mt-3 text-2xl font-black tracking-[-.03em] text-white">Tune the destination.</h2><p className="mt-1 text-sm leading-6 text-slate-400">Update the name or goal without losing the planning context.</p></div><button type="button" onClick={() => onOpenChange(false)} disabled={busy} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-white/10 bg-white/[.03] text-slate-500 hover:bg-white/[.07] hover:text-white disabled:opacity-40" aria-label="Close editor"><X className="h-4 w-4" /></button></div>
        ...
        <div className="mt-5 rounded-2xl border border-cyan-200/10 bg-cyan-200/[.025] p-4"><div className="flex items-start gap-2"><Sparkles className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" /><div><div className="text-xs font-black uppercase tracking-[.16em] text-cyan-100">AI goal context</div><p className="mt-1.5 text-[11px] leading-5 text-slate-500">The detailed goal remains available to AI planning while the switcher stays clean.</p></div></div></div>
        ...
        <div className="mt-6 grid gap-3 rounded-2xl border border-white/10 bg-[#050d18] p-3 sm:grid-cols-[1fr_auto] sm:items-center sm:p-4"><div><p className="text-[9px] font-black uppercase tracking-[.16em] text-slate-500">Ready to update?</p><p className="mt-1 text-xs text-slate-600">Your current edits will be saved to this roadmap.</p></div><div className="flex gap-2 sm:justify-end"><button type="button" onClick={() => onOpenChange(false)} disabled={busy} className="flex-1 rounded-xl border border-white/10 bg-white/[.02] px-4 py-3 text-sm font-bold text-slate-300 hover:bg-white/[.06] disabled:opacity-40 sm:flex-none">Cancel</button><button type="button" disabled={!canSave} onClick={() => void onSave({ title: cleanTitle, goal: cleanGoal })} className="flex-1 rounded-xl bg-gradient-to-r from-cyan-300 via-sky-300 to-violet-300 px-5 py-3 text-sm font-black text-slate-950 shadow-[0_14px_36px_-22px_rgba(34,211,238,.85)] transition hover:brightness-105 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-cyan-200/20 disabled:cursor-not-allowed disabled:opacity-35 sm:flex-none">{saving ? "Saving…" : "Save changes"}</button></div></div>
      </div>
    </motion.div>
