...
export function RoadmapEditDialog({ open, initial, roadmapId, onClose, onSave, onSmartChange, onDelete, saving, askingAI, deleting }: Props) {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape" && !saving && !askingAI && !deleting) {
        if (confirmDelete) setConfirmDelete(false); else onClose();
      }
    };
    window.addEventListener("keydown", onKeyDown);
    return () => window.removeEventListener("keydown", onKeyDown);
  }, [open, onClose, saving, askingAI, deleting, confirmDelete]);
  ...
  const handleDelete = async () => {
    ...
    try { await onDelete(); } catch (error) { toast.error(error instanceof Error ? error.message : "Could not delete roadmap."); }
  };

  return <AnimatePresence>
    {open && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[90] flex items-end justify-center bg-[#020617]/85 p-0 backdrop-blur-md sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="roadmap-edit-title" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) onClose(); }}>
      <motion.div initial={{ opacity: 0, y: 18, scale: .985 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 12, scale: .985 }} transition={{ duration: .2, ease: [0.22, 1, .36, 1] }} className="relative w-full max-w-xl overflow-hidden rounded-t-[2rem] border border-cyan-300/10 bg-[#07101d] shadow-[0_40px_120px_-50px_rgba(34,211,238,.45)] sm:rounded-[2rem]" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="relative p-5 sm:p-7">
          <div className="flex items-start justify-between gap-4"><div><div className="inline-flex items-center gap-2 rounded-full border border-cyan-200/20 bg-cyan-200/10 px-3 py-1 text-[9px] font-black uppercase tracking-[.18em] text-cyan-100"><Pencil className="h-3 w-3" /> Edit roadmap</div><h2 id="roadmap-edit-title" className="mt-3 text-2xl font-black tracking-[-.03em] text-white">Tune the destination.</h2><p className="mt-1 text-sm leading-6 text-slate-400">Keep the public label simple. OUTSTAND keeps the detailed planning context behind the scenes.</p></div><button type="button" onClick={onClose} disabled={busy} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-white/10 bg-white/[0.03] text-slate-500 transition-colors hover:bg-white/[0.07] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-200 disabled:opacity-40" aria-label="Close editor"><X className="h-4 w-4" /></button></div>

          <div className="mt-6 space-y-4">
            <label className="block"><span className="flex items-center justify-between text-[10px] font-black uppercase tracking-[.18em] text-slate-400"><span>Roadmap name</span><span className="text-slate-600">{title.length}/60</span></span><input ref={titleRef} value={title} onChange={(e) => setTitle(e.target.value)} maxLength={60} autoComplete="off" placeholder="e.g. Road to 1500" className="mt-2 w-full rounded-2xl border border-white/10 bg-[#050d18] px-4 py-3.5 text-base font-black tracking-tight text-white outline-none transition focus:border-cyan-300/50 focus:ring-4 focus:ring-cyan-300/[0.07]" /><p className="mt-1.5 text-[10px] text-slate-600">Aim for a short, recognizable name.</p></label>
          </div>

          <div className="mt-5 rounded-2xl border border-cyan-200/10 bg-cyan-200/[0.025] p-4"><div className="flex items-start gap-2"><Sparkles className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200" /><div><div className="text-xs font-black uppercase tracking-[.16em] text-cyan-100">AI goal context</div><p className="mt-1.5 text-[11px] leading-5 text-slate-500">Hidden from the roadmap switcher. This is the detailed outcome AI uses to evaluate and adapt the plan.</p></div></div></div>
          ...
          {confirmDelete ? <div className="mt-5 rounded-2xl border border-red-400/20 bg-red-400/[0.045] p-4"><div className="flex items-start gap-3"><div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-red-400/10 text-red-300"><AlertTriangle className="h-4 w-4" /></div><div className="min-w-0"><div className="text-sm font-black text-white">Delete this roadmap?</div><p className="mt-1 text-[11px] leading-5 text-slate-400">This removes the roadmap and its planning data. This action cannot be undone.</p></div></div><div className="mt-4 flex gap-2"><button type="button" onClick={() => setConfirmDelete(false)} disabled={busy} className="flex-1 rounded-xl border border-white/10 bg-white/[.02] px-4 py-3 text-sm font-bold text-slate-300 hover:bg-white/[.06] disabled:opacity-40">Cancel</button><button type="button" onClick={() => void handleDelete()} disabled={busy} className="flex-1 rounded-xl bg-red-400/15 px-4 py-3 text-sm font-black text-red-200 ring-1 ring-inset ring-red-300/20 hover:bg-red-400/20 disabled:opacity-40">{deleting ? "Deleting…" : "Delete roadmap"}</button></div></div> : <button type="button" onClick={() => setConfirmDelete(true)} disabled={busy} className="mt-5 inline-flex items-center gap-2 rounded-xl px-1 py-2 text-xs font-black text-red-300/80 transition hover:text-red-200 disabled:opacity-40"><Trash2 className="h-4 w-4" /> Delete roadmap</button>}

          <div className="mt-5 flex items-center gap-2 rounded-xl border border-cyan-200/10 bg-cyan-200/[0.025] px-3 py-2.5 text-[11px] leading-5 text-slate-400"><Check className="h-3.5 w-3.5 shrink-0 text-cyan-300" /> The detailed goal stays available to AI without cluttering the roadmap list.</div>
