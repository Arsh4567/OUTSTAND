...
export function RoadmapEditDialog({ open, initial, onClose, onSave, onAskAI, saving, askingAI }: { open: boolean; initial: RoadmapEditPatch; onClose: () => void; onSave: (patch: RoadmapEditPatch) => void; onAskAI: (request: string) => void; saving?: boolean; askingAI?: boolean }) {
  ...
  useEffect(() => { if (!open) return; setTitle(initial.title); setGoal(initial.goal); setRequest(""); }, [open, initial.title, initial.goal]);
  if (!open) return null;
  return <div className="fixed inset-0 z-[90] flex items-end justify-center bg-black/70 p-0 backdrop-blur-md sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="roadmap-edit-title">
    <div className="w-full max-w-xl rounded-t-[2rem] border border-white/10 bg-[#07101f] p-5 shadow-2xl sm:rounded-[2rem] sm:p-7">
      ...
      <div className="mt-6 space-y-4">
        ...
        <div className="rounded-2xl border border-violet-300/15 bg-violet-300/[0.045] p-4"><div className="flex items-center gap-2 text-violet-200"><Sparkles className="h-4 w-4" /><span className="text-xs font-black uppercase tracking-[.16em]">Ask AI to change it</span></div><textarea value={request} onChange={(e) => setRequest(e.target.value)} maxLength={500} rows={3} placeholder="e.g. Make the plan shorter, focus more on practice, and move the heavy sessions to weekends." className="mt-3 w-full resize-none rounded-xl border border-white/10 bg-black/20 p-3 text-sm text-white outline-none placeholder:text-slate-700 focus:border-violet-300/30" /><button type="button" disabled={askingAI || request.trim().length < 5} onClick={() => onAskAI(request.trim())} className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-violet-300/20 bg-violet-300/[0.08] px-4 py-3 text-xs font-black text-violet-100 disabled:opacity-40"><Sparkles className="h-4 w-4" />{askingAI ? "AI is reshaping your roadmap…" : "Apply AI changes"}</button></div>
      </div>
      <div className="mt-6 flex gap-2"><button type="button" onClick={onClose} className="flex-1 rounded-xl border border-white/10 px-4 py-3 text-sm font-bold text-slate-400 hover:bg-white/5 hover:text-white">Cancel</button><button type="button" disabled={saving || title.trim().length < 2 || goal.trim().length < 5} onClick={() => onSave({ title: title.trim(), goal: goal.trim() })} className="flex-1 rounded-xl bg-cyan-300 px-4 py-3 text-sm font-black text-slate-950 hover:bg-cyan-200 disabled:opacity-40">{saving ? "Saving…" : "Save changes"}</button></div>
    </div>
  </div>;
}
