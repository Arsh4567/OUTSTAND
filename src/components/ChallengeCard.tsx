...
export function ChallengeCard({ challenge, completionStage, running, mins, secs, setRunning, setRemaining, generate, complete }: ChallengeCardProps) {
  ...
  const totalMinsInt = typeof mins === "number" ? mins : parseInt(String(mins), 10) || 0;
  const secsInt = typeof secs === "number" ? secs : parseInt(String(secs), 10) || 0;
  ...
  const timerColor = useMemo(() => {
    ...
    return `hsl(${Math.round(ratio * 180 + 8)}, 92%, 62%)`;
  ...
  const handleMouseMove = (event: React.MouseEvent<HTMLDivElement>) => {
    if (completionStage !== 0 || event.nativeEvent.pointerType === "touch") return;
  ...
  return (
    ...
    >
      <motion.div
        ...
        animate={completionStage === 1 ? { scale: 0.96, y: 0, rotateZ: [0, -1.2, 1.2, 0], filter: "brightness(1.25)" } : completionStage === 2 ? { scale: [0.96, 1.04, 0.98], y: [0, -10, -640], opacity: [1, 1, 0], filter: "brightness(1.8)" } : { opacity: 1, scale: 1, y: 0 }}
        ...
        className={cn("relative flex w-full items-center justify-center", completionStage > 0 ? "h-24 max-w-sm" : "min-h-[460px] sm:min-h-[520px] md:min-h-[590px]")}
      >
        ...
        <motion.div animate={completionStage > 0 ? { opacity: 0, scale: 0.8 } : { opacity: 1, scale: 1 }} className={cn("relative z-10 flex w-full flex-col items-center text-center", completionStage === 0 ? "p-7 sm:p-10 md:p-14" : "p-0")} style={completionStage === 0 ? { transform: "translateZ(48px)" } : undefined}>
          ...
          <div className="my-8 sm:my-10"><div className="rounded-[1.6rem] border border-white/[0.07] bg-black/20 px-7 py-3 backdrop-blur-md sm:px-10 sm:py-4"><motion.span animate={isUrgent && running ? { scale: [1, 1.035, 1] } : { scale: 1 }} transition={{ duration: 1, repeat: isUrgent && running ? Infinity : 0, ease: "easeInOut" }} className="inline-block font-mono text-5xl font-light tracking-[-0.06em] tabular-nums sm:text-6xl md:text-7xl" style={{ color: timerColor, textShadow: `0 0 28px ${timerColor}35` }}>{formatTime()}</motion.span></div></div>
          <div className="mb-8 flex items-center justify-center gap-4 sm:gap-6">
            <motion.button type="button" whileHover={{ scale: 1.07 }} whileTap={{ scale: 0.94 }} onClick={() => { setRemaining(durationFallback * 60); setRunning(false); }} className="flex h-12 w-12 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] text-zinc-400 transition-colors hover:bg-white/[0.08] hover:text-white sm:h-14 sm:w-14" aria-label="Reset challenge"><RotateCcw className="h-5 w-5" /></motion.button>
            <motion.button type="button" whileHover={{ scale: 1.045 }} whileTap={{ scale: 0.95 }} onClick={() => setRunning(!running)} disabled={isFinished} className="relative flex h-20 w-20 items-center justify-center overflow-hidden rounded-full border shadow-[0_15px_50px_-18px_rgba(34,211,238,0.5)] sm:h-24 sm:w-24 md:h-28 md:w-28 disabled:cursor-not-allowed disabled:opacity-50" style={{ backgroundColor: running ? "rgba(255,255,255,0.08)" : `${timerColor}20`, borderColor: running ? "rgba(255,255,255,0.16)" : `${timerColor}55`, color: running ? "#fff" : timerColor }} aria-label={running ? "Pause challenge" : "Start challenge"}><span className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_35%_25%,rgba(255,255,255,0.18),transparent_35%)]" />{running ? <Pause className="relative z-10 h-8 w-8 fill-current sm:h-10 sm:w-10" /> : <Play className="relative z-10 ml-1 h-8 w-8 fill-current sm:h-10 sm:w-10" />}</motion.button>
            <motion.button type="button" whileHover={{ scale: 1.07 }} whileTap={{ scale: 0.94 }} onClick={generate} className="flex h-12 w-12 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] text-zinc-400 transition-colors hover:bg-white/[0.08] hover:text-white sm:h-14 sm:w-14" aria-label="Skip challenge"><SkipForward className="h-5 w-5" /></motion.button>
          </div>
          ...
          <div className="mt-4 w-full"><Button onClick={complete} disabled={!isFinished} className={cn("h-14 w-full rounded-2xl text-sm font-bold uppercase tracking-[0.16em] transition-all sm:h-16", isFinished ? "bg-white text-black shadow-[0_16px_50px_-18px_rgba(255,255,255,0.65)] hover:scale-[1.01] hover:bg-cyan-50" : "border border-white/8 bg-white/[0.04] text-zinc-600")}>{isFinished ? <><CheckCircle2 className="mr-2 h-4 w-4" /> Complete mission</> : "Complete when timer reaches zero"}</Button></div>
        </motion.div>
...
function InfoTile({ label, value, accent, className }: { label: string; value: string; accent?: string; className?: string }) {
  return <div className={cn("rounded-2xl border border-white/[0.06] bg-black/15 px-4 py-3 text-left", className)}><div className="text-[9px] font-bold uppercase tracking-[0.18em] text-zinc-600">{label}</div><div className="mt-1 truncate text-sm font-semibold text-zinc-200" style={accent ? { color: accent } : undefined}>{value}</div></div>;
}
