...
const HighYieldBank: React.FC = () => {
  ...
  return (
    ...
    <div className="relative w-full min-h-[90vh] bg-[#02040a] text-slate-200 overflow-hidden flex flex-col">
      ...
      <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
        <div 
          className="absolute -left-16 top-10 w-72 h-[400px] bg-gradient-to-b from-blue-600/30 to-transparent rotate-45 backdrop-blur-3xl"
          style={{ clipPath: 'polygon(50% 0%, 100% 50%, 50% 100%, 0% 50%)', boxShadow: '0 0 150px 30px rgba(59, 130, 246, 0.4)' }} 
        />
        <div 
          ...
          style={{ clipPath: 'polygon(50% 0%, 100% 50%, 50% 100%, 0% 50%)', boxShadow: '0 0 150px 30px rgba(79, 70, 229, 0.3)' }} 
        />
      </div>
        ...
        <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="px-4 sm:px-8 py-8 space-y-8">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 px-4 py-2 bg-[#0a0f1a]/80 backdrop-blur-md rounded-xl border border-white/10 w-fit shadow-lg">
                ...
                <div className="flex space-x-2 p-1.5 bg-[#0a0f1a]/80 backdrop-blur-md rounded-2xl border border-white/5 shadow-inner w-full sm:w-fit">
                  <button onClick={() => setActiveSubject('Science')} className={`relative flex-1 sm:flex-none flex items-center justify-center gap-2.5 px-6 py-3 rounded-xl text-sm font-bold transition-all duration-300 ${activeSubject === 'Science' ? 'text-white' : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'}`}>
                    {activeSubject === 'Science' && <motion.div layoutId="activeTabBg" className="absolute inset-0 bg-gradient-to-r from-blue-600/30 to-indigo-600/30 border border-blue-500/40 rounded-xl shadow-[0_0_20px_rgba(59,130,246,0.2)]" />}
                  ...
                  <button onClick={() => setActiveSubject('Mathematics')} className={`relative flex-1 sm:flex-none flex items-center justify-center gap-2.5 px-6 py-3 rounded-xl text-sm font-bold transition-all duration-300 ${activeSubject === 'Mathematics' ? 'text-white' : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'}`}>
                    {activeSubject === 'Mathematics' && <motion.div layoutId="activeTabBg" className="absolute inset-0 bg-gradient-to-r from-blue-600/30 to-indigo-600/30 border border-blue-500/40 rounded-xl shadow-[0_0_20px_rgba(59,130,246,0.2)]" />}
              ...
              <div className="w-full flex-1 border-t border-white/10 bg-black/20 backdrop-blur-sm">
                {Object.entries(groupedQuestions).map(([chapter, questions]) => {
                  ...
                  return (
                    <motion.button
                      ...
                      // Truly edge-to-edge with solid separation lines
                      className="w-full flex items-center justify-between px-4 sm:px-8 py-5 border-b border-white/10 transition-colors group text-left gap-4"
                    >
                      {/* Full Text Title */}
                      <div className="flex-1 min-w-0 pr-4">
                        <h3 className="text-white font-bold text-base sm:text-lg whitespace-normal break-words leading-tight group-hover:text-blue-300 transition-colors">
                          {chapter}
                        </h3>
                        ...
                        <div className="flex flex-col items-end gap-1.5 w-16">
                          ...
                          <div className="w-full h-1.5 bg-[#1a1f2e] rounded-full overflow-hidden shadow-inner">
                            <motion.div 
                              ...
                              className={`h-full rounded-full ${isFullyMastered ? 'bg-emerald-400' : 'bg-gradient-to-r from-blue-500 to-indigo-400'}`} 
                            />
                          </div>
                        </div>
          ...
          /* ========================================= */
            ...
            >
              ...
              <div className="sticky top-0 z-30 bg-[#02040a]/90 backdrop-blur-2xl px-4 sm:px-8 py-4 border-b border-white/10 flex items-center gap-4 shadow-lg">
                <button onClick={() => setActiveChapter(null)} className="p-2.5 bg-[#0a0f1a] hover:bg-white/10 rounded-xl transition-colors border border-white/10 group flex-shrink-0">
              ...
              <div className="px-4 sm:px-8 py-8 grid gap-6 pb-20">
                {groupedQuestions[activeChapter].map((q, index) => (
                  <QuestionCard key={q.id} question={q} index={index + 1} isCompleted={completedQs.includes(q.id)} onToggle={() => toggleCompletion(q.id)} />
                ))}
              </div>
            </motion.div>
          )}
...
const QuestionCard: React.FC<QuestionCardProps> = ({ question, index, isCompleted, onToggle }) => {
  ...
  return (
    <motion.div layout className={`relative p-5 sm:p-7 rounded-[1.5rem] border backdrop-blur-md transition-all duration-300 ${isCompleted ? 'bg-[#060a14]/90 border-emerald-500/30 shadow-[0_0_40px_rgba(52,211,153,0.05)]' : 'bg-[#0a0f1a]/80 border-white/10 shadow-2xl'}`}>
      <div className="flex justify-between items-start mb-6">
        <div className="flex flex-wrap items-center gap-3">
          <span className={`inline-flex items-center justify-center h-9 w-9 rounded-xl font-black text-sm shadow-inner ${isCompleted ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-blue-600/30 text-blue-400 border border-blue-500/40 shadow-[0_0_15px_rgba(59,130,246,0.3)]'}`}>
            Q{index}
          </span>
        ...
        <button onClick={onToggle} className="relative group p-1">
          ...
          {isCompleted ? <CheckCircle2 className="w-8 h-8 text-emerald-400 relative z-10 drop-shadow-[0_0_10px_rgba(52,211,153,0.8)]" /> : <Circle className="w-8 h-8 text-slate-500 group-hover:text-blue-400 relative z-10 transition-colors" />}
        </button>
      </div>

      <h3 className={`text-base sm:text-lg font-medium mb-6 whitespace-pre-line leading-relaxed transition-colors ${isCompleted ? 'text-slate-300' : 'text-slate-50'}`}>
      ...
      {isSubjective && (
        <div className="mb-2">
          {!isRevealed && (
            <button onClick={() => setIsRevealed(true)} className="flex items-center gap-2.5 py-3 px-5 rounded-xl bg-blue-600/20 border border-blue-500/40 text-blue-300 font-bold hover:bg-blue-500/30 hover:text-white hover:shadow-[0_0_20px_rgba(59,130,246,0.3)] transition-all group">
              <Check className="w-5 h-5 group-hover:scale-110 transition-transform" /> Reveal Official Solution
            </button>
          )}
        </div>
      )}
          ...
          <motion.div initial={{ opacity: 0, height: 0, y: -10 }} animate={{ opacity: 1, height: 'auto', y: 0 }} exit={{ opacity: 0, height: 0, y: -10 }} className="overflow-hidden">
            <div className="mt-6 p-5 rounded-2xl bg-[#050810]/90 border border-white/10 relative shadow-inner">
              ...
              <div className="flex items-center gap-2 mb-3 text-blue-400 font-bold text-sm tracking-wide uppercase ml-3">
                <BookOpen className="w-4 h-4" /> Official Solution & Analysis
              </div>
