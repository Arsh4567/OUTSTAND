...
async function loadRoadmapContext(userId: string): Promise<RoadmapContext | null> {
  ...
  const [{ data: milestones }, { data: tasks }] = await Promise.all([
    ...
    supabase
      .from("roadmap_tasks")
      .select("id,title,instructions,estimated_minutes,start_time,end_time,success_criteria,is_required,roadmap_task_progress(status)")
      .eq("roadmap_id", roadmap.id)
      .eq("user_id", userId)
  ...
  const progressRows = (tasks ?? []).map((task: any) => {
    const relation = Array.isArray(task.roadmap_task_progress) ? task.roadmap_task_progress[0] : task.roadmap_task_progress;
    return {
      id: String(task.id),
      title: String(task.title || "Untitled task"),
      instructions: String(task.instructions || task.title || "Complete the task."),
      estimatedMinutes: typeof task.estimated_minutes === "number" ? task.estimated_minutes : null,
      startTime: task.start_time ? String(task.start_time) : null,
      endTime: task.end_time ? String(task.end_time) : null,
      successCriteria: task.success_criteria ? String(task.success_criteria) : null,
      progress: String(relation?.status || "pending"),
  ...
  return {
    ...
    milestones: (milestones ?? []).map((milestone: any) => ({
      ...
      dayEnd: Number(milestone.day_end) || 1,
    })),
    todayTasks: progressRows,
  };
}
...
export function ChatAssistant({ openSignal = false, onOpenSignalHandled }: ChatAssistantProps) {
  ...
  }), [user, profile, habits, sessions, outstand, xp, bestStreak, log, today, roadmapContext]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void loadRoadmapContext(user.id)
      .then((value) => { if (!cancelled) setRoadmapContext(value); })
      .catch((error) => console.error("AI roadmap context load failed", error));
  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)} aria-label="Open Outstand Intelligence" className="fixed bottom-5 right-5 z-[80] hidden h-12 w-12 place-items-center rounded-2xl border border-cyan-300/20 bg-[#050812]/95 text-cyan-200 shadow-[0_0_30px_rgba(34,211,238,.16)] backdrop-blur-xl transition hover:scale-105 hover:border-cyan-300/35 hover:bg-cyan-300/10 md:grid">
        <span className="relative grid place-items-center"><span className="absolute h-3 w-3 animate-ping rounded-full bg-cyan-300/30" /><span className="relative h-2.5 w-2.5 rounded-full bg-cyan-300 shadow-[0_0_12px_rgba(103,232,249,.9)]" /></span>
      </button>
      ...
      <Drawer open={open} onOpenChange={setOpen}>
        <DrawerContent className="z-[90] h-[92vh] max-h-[920px] border-white/10 bg-[#050812]/98 p-0 text-white shadow-[0_-30px_100px_rgba(0,0,0,.55)] backdrop-blur-2xl">
          <div className="h-full min-h-0"><OutstandChatPanel initialMessages={messages} context={context} onClose={() => setOpen(false)} onClear={clearLocalHistory} historyLoading={historyLoading} /></div>
        </DrawerContent>
      </Drawer>
    </>
  );
