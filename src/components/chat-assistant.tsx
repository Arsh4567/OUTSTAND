...
type RoadmapTaskWithProgress = {
  ...
  is_required: boolean;
  roadmap_task_progress: { status: string } | { status: string }[] | null;
};
...
async function loadRoadmapContext(userId: string): Promise<RoadmapContext | null> {
  ...
  const [{ data: milestonesData }, { data: tasks }] = await Promise.all([
    supabase
    ...
    supabase
      .from("roadmap_tasks")
      .select(
        "id,title,instructions,estimated_minutes,start_time,end_time,success_criteria,is_required,roadmap_task_progress(status)",
      )
      .eq("roadmap_id", roadmap.id)
      .eq("user_id", userId)
  ...
  const todayTasks = ((tasks as unknown as RoadmapTaskWithProgress[]) ?? []).map((task) => {
    const relation = Array.isArray(task.roadmap_task_progress)
      ? task.roadmap_task_progress[0]
  ...
  return {
    ...
    todayDay,
    milestones: ((milestonesData as any[]) ?? []).map((milestone) => ({
...
export function ChatAssistant({ openSignal = false, onOpenSignalHandled }: ChatAssistantProps) {
  ...
  const context = useMemo<OutstandChatContext>(
    () =>
      ({
        ...
        ...(roadmapContext ? { roadmap: roadmapContext } : {}),
      }) as OutstandChatContext,
  ...
  useEffect(() => {
    ...
    void loadRoadmapContext(user.id)
      .then((value) => {
        if (!cancelled) setRoadmapContext(value);
      })
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      ...
      try {
        ...
        const loaded: UIMessage[] = ((messageData as any[]) ?? []).map((message) => ({
