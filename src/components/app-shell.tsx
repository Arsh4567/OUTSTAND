...
function ShellWithChrome() {
  ...
  const handleNotificationToggle = async () => {
    if (!isNotificationsGranted) {
      // requestNotificationPermission now throws when the subscription cannot
      // be created/saved, so the UI never claims success falsely.
      await requestNotificationPermission();
      setIsNotificationsGranted(true);
      return;
    }
    alert("To disable notifications, please use the notification controls in Settings or your browser/device settings.");
  };
  ...
  const handleTestPush = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Network error while trying to send test push.");
    } finally {
  ...
  return (
    <div className="relative flex min-h-screen flex-col overflow-x-hidden bg-transparent pb-0 text-slate-50 transition-colors duration-300">
      <AnimatePresence>
        {showSupernova && <SupernovaEffect level={level} onClose={() => setShowSupernova(false)} />}
      </AnimatePresence>
      ...
      <AnimatePresence mode="wait">
        <motion.main key={pathname} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} transition={{ duration: 0.3, ease: "easeOut" }} className="mx-auto w-full max-w-7xl flex-1 px-4 py-6 relative z-10">
          <Outlet />
        </motion.main>
      </AnimatePresence>
...
function SupernovaEffect({ level, onClose }: { level: number; onClose: () => void }) {
  const particles = Array.from({ length: 45 }).map((_, i) => {
    ...
    return { id: i, x: Math.cos(angle) * distance, y: Math.sin(angle) * distance, size: Math.random() * 10 + 4, color: Math.random() > 0.5 ? "#fbbf24" : "#60a5fa" };
  });
  ...
  return (
    <motion.div className="fixed inset-0 z-[100] flex flex-col items-center justify-center pointer-events-none overflow-hidden" initial={{ backgroundColor: "rgba(255, 255, 255, 1)" }} animate={{ backgroundColor: "rgba(3, 7, 18, 0.85)" }} exit={{ opacity: 0 }} transition={{ duration: 0.8, ease: "easeOut" }}>
      ...
      <motion.div className="relative z-10 text-center flex flex-col items-center" initial={{ scale: 0.5, opacity: 0, y: 50 }} animate={{ scale: 1, opacity: 1, y: 0 }} transition={{ delay: 0.2, type: "spring", bounce: 0.6 }}>
        <motion.div className="text-yellow-400 font-black tracking-widest uppercase mb-2 drop-shadow-[0_0_30px_rgba(250,204,21,0.6)]" style={{ fontSize: "clamp(3rem, 10vw, 6rem)" }} animate={{ scale: [1, 1.05, 1] }} transition={{ repeat: Infinity, duration: 2 }}>Level Up</motion.div>
        <div className="bg-white/10 border border-white/20 backdrop-blur-md px-6 py-2 rounded-full text-white font-medium text-lg sm:text-2xl tracking-wide shadow-2xl">You reached Level <span className="text-blue-400 font-bold">{level}</span></div>
      </motion.div>
    </motion.div>
  );
}
