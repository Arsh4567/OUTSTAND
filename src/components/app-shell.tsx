...
function ShellWithChrome() {
  ...
  const handleNotificationToggle = async () => {
    if (!isNotificationsGranted) { await requestNotificationPermission(); setIsNotificationsGranted(true); return; }
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-transparent text-slate-50">
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-[1800px]">
        <DesktopSidebar pathname={pathname} level={level} xp={safeXp} onSettings={() => setIsSettingsOpen(true)} />
