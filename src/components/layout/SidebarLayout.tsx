              ...
              <motion.aside initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }} transition={{ ease: customEase, duration: 0.48 }} className="fixed inset-y-0 left-0 z-[60] flex w-[86vw] max-w-[360px] flex-col overflow-hidden border-r border-white/10 bg-[#060911]/95 shadow-[30px_0_80px_rgba(0,0,0,0.55)] backdrop-blur-2xl sm:max-w-[390px]" aria-label="Primary navigation">
                ...
                <nav className="relative z-10 flex-1 overflow-y-auto px-4 py-6" aria-label="Main navigation"><div className="mb-3 px-3 text-[10px] font-black uppercase tracking-[0.24em] text-slate-600">Workspace</div><div className="space-y-1.5">
                  {NAV_ITEMS.map((item, index) => {
                    ...
                    return <motion.div key={item.to} initial={{ opacity: 0, x: -14 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.04 + index * 0.035, duration: 0.28, ease: customEase }}><Link to={item.to} className={cn("group relative flex items-center gap-3 overflow-hidden rounded-2xl border px-3 py-3 transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/70", isActive ? "border-cyan-300/15 bg-gradient-to-r from-cyan-400/10 via-blue-500/[0.06] to-transparent shadow-[inset_0_0_24px_rgba(34,211,238,0.04)]" : "border-transparent hover:border-white/[0.045] hover:bg-white/[0.045]")}>
                      ...
                      <div className="min-w-0 flex-1"><div className={cn("text-sm font-bold", isActive ? "text-white" : "text-slate-300 group-hover:text-white")}>{item.label}</div><div className={cn("mt-0.5 text-[10px] font-medium", isActive ? "text-cyan-200/70" : "text-slate-600 group-hover:text-slate-500")}>{isActive ? "Currently active" : "Open section"}</div></div><ChevronRight className={cn("h-4 w-4 transition-all", isActive ? "text-cyan-200/70" : "text-slate-700 group-hover:translate-x-0.5 group-hover:text-slate-500")} />
                    </Link></motion.div>;
                  })}
                </div></nav>
