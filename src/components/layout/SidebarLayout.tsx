  ...
  return (
    <MotionConfig reducedMotion="user">
      <div className="min-h-screen bg-[#050508] font-sans text-slate-100 selection:bg-cyan-400/20 selection:text-cyan-100">
        <header className="sticky top-0 z-40 border-b border-white/[0.06] bg-[#030508]/80 backdrop-blur-2xl">
          <div className="mx-auto flex h-[72px] w-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-center gap-2">
              <Link to="/league" aria-label="Open leaderboard" className="group hidden min-w-[190px] items-center gap-3 rounded-2xl border border-cyan-300/15 bg-white/[0.035] px-3 py-2 text-left transition hover:border-cyan-300/30 hover:bg-cyan-300/[0.06] md:flex"><div className="grid h-8 w-8 shrink-0 place-items-center rounded-xl border border-amber-300/15 bg-amber-300/[0.06] text-amber-200"><Trophy className="h-4 w-4" /></div><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><span className="text-[9px] font-black uppercase tracking-[0.16em] text-slate-500">Level {level}</span><span className="text-[10px] font-black text-cyan-200 tabular-nums">{safeXp.toLocaleString()} XP</span></div><div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-white/[0.06]"><motion.div initial={{ scaleX: 0 }} animate={{ scaleX: Math.min(1, Math.max(0, progressPct / 100)) }} transition={{ duration: 0.7, ease: "easeOut" }} style={{ transformOrigin: "left center" }} className="h-full w-full origin-left rounded-full bg-gradient-to-r from-cyan-400 via-blue-400 to-violet-400 will-change-transform" /></div><div className="mt-1 text-[8px] font-bold uppercase tracking-[0.12em] text-slate-600">Tap XP to view leaderboard</div></div></Link>
              <Link to="/profile" className="group flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.035] px-2.5 py-1.5 transition hover:border-cyan-300/20 hover:bg-white/[0.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/70" aria-label="Open profile"><div className="relative h-9 w-9 overflow-hidden rounded-xl border border-cyan-300/20 bg-cyan-950/40">{profile?.avatar_url ? <img src={profile.avatar_url} alt="" className="h-full w-full object-cover" /> : <div className="grid h-full w-full place-items-center text-xs font-black text-cyan-200">{initial}</div>}<span className="absolute bottom-0.5 right-0.5 h-2.5 w-2.5 rounded-full border-2 border-[#030508] bg-emerald-400" /></div><div className="hidden text-right md:block"><div className="max-w-32 truncate text-xs font-bold text-white">{safeName}</div><div className="mt-0.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500">Level {level}</div></div></Link>
            </div>
          </div>
        </header>
        ...
        <AnimatePresence>{isOpen && <><motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsOpen(false)} className="fixed inset-0 z-50 bg-black/70 backdrop-blur-md" /><motion.aside initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }} transition={{ ease: customEase, duration: 0.42 }} className="fixed inset-y-0 left-0 z-[60] flex w-[86vw] max-w-[360px] flex-col overflow-hidden border-r border-white/10 bg-[#060911]/95 shadow-[30px_0_80px_rgba(0,0,0,0.55)] backdrop-blur-2xl" aria-label="Primary navigation">
          ...
          <div className="relative z-10 flex items-center justify-end border-t border-white/7 bg-[#060911]/80 p-4 backdrop-blur-xl"><button type="button" onClick={() => { setIsOpen(false); setIsSettingsOpen(true); }} aria-label="Open settings" className="grid h-10 w-10 place-items-center rounded-full border border-white/10 bg-white/[0.04] text-slate-500 transition hover:border-cyan-300/20 hover:bg-cyan-300/[0.08] hover:text-cyan-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/70"><Settings className="h-4 w-4" /></button></div>
