    ...
    <MotionConfig reducedMotion="user">
      <div className="min-h-screen bg-background font-sans text-foreground selection:bg-primary/20 selection:text-primary">
        ...
        <AnimatePresence initial={false}>
          {isOpen ? (
            <>
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsOpen(false)} className="fixed inset-0 z-50 bg-foreground/60 backdrop-blur-md" aria-hidden="true" />
              <motion.aside id="primary-navigation" initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }} transition={{ ease: customEase, duration: 0.3 }} className="fixed inset-y-0 left-0 z-[60] flex w-[min(88vw,360px)] flex-col overflow-hidden border-r border-border bg-background/95 pb-[env(safe-area-inset-bottom)] shadow-2xl backdrop-blur-2xl" aria-label="Primary navigation">
                <div className="relative z-10 flex items-center justify-between border-b border-border px-4 py-4 sm:px-5">
                  ...
                  <button ref={navCloseButtonRef} type="button" onClick={() => setIsOpen(false)} aria-label="Close navigation" className="grid h-11 w-11 place-items-center rounded-xl border border-border bg-surface-2 text-muted-foreground transition hover:border-primary/30 hover:bg-surface-3 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.98]"><X className="h-5 w-5" /></button>
                </div>
                ...
                <nav className="relative z-10 flex-1 overflow-y-auto overscroll-contain px-4 py-5 sm:py-6" aria-label="Main navigation">
                  ...
                  <div className="space-y-1.5">
                    {NAV_ITEMS.map((item) => {
                      ...
                      return <Link key={item.to} to={item.to} onClick={() => setIsOpen(false)} aria-current={active ? "page" : undefined} className={cn("group relative flex min-h-[56px] items-center gap-3 rounded-2xl border px-3 py-2.5 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.99]", active ? "border-primary/20 bg-primary/10 text-foreground" : "border-transparent text-muted-foreground hover:border-border hover:bg-surface-2 hover:text-foreground")}>
                        <div className={cn("grid h-10 w-10 shrink-0 place-items-center rounded-xl border", active ? "border-primary/20 bg-primary/10 text-primary" : "border-border bg-surface-2 text-muted-foreground group-hover:text-foreground")}><item.icon className="h-[18px] w-[18px]" aria-hidden="true" /></div>
                        ...
                        <ChevronRight className="h-4 w-4 shrink-0 text-muted-foreground/70 transition-transform duration-200 group-hover:translate-x-0.5 group-hover:text-muted-foreground" aria-hidden="true" />
                      </Link>;
                    })}
                  </div>
                </nav>
