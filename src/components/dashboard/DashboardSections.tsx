...
export function DashboardWelcome({ name, quote }: { name: string; quote: { quote: string; author: string } }) {
  return <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .55, ease }} className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
    ...
    <div className="flex shrink-0 items-center gap-2 rounded-2xl border border-emerald-300/10 bg-emerald-300/[0.025] px-3 py-2 text-[10px] font-bold uppercase tracking-[0.16em] text-slate-500" title="Your dashboard is ready and up to date"><span className="h-2 w-2 rounded-full bg-emerald-400 shadow-[0_0_12px_rgba(52,211,153,.7)]" /> Ready for today</div>
  </motion.section>;
}
...
export function DashboardIntelligence({ habits, sessions, completedHabits, focusMinutes, bestStreak, nextMission }: { habits: Habit[]; sessions: FocusSession[]; completedHabits: number; focusMinutes: number; bestStreak: number; nextMission?: DashboardMission }) {
  ...
  const weakHabit = habits.find((habit) => !habit.history?.includes(today));
  ...
  const actionTo = !hasSystem ? "/onboarding" : "/focus";
  ...
  return <motion.section initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .08, duration: .5, ease }} className={`${panel} relative overflow-hidden p-5 sm:p-7`}>
    ...
    <div className="relative mt-6 flex flex-col gap-4 border-t border-white/[0.07] pt-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl border border-violet-300/15 bg-violet-300/[0.07] text-violet-200"><Lightbulb className="h-4 w-4" /></span><div><p className="text-[9px] font-black uppercase tracking-[0.18em] text-slate-600">Recommended next move</p><p className="mt-1 text-sm font-bold text-white">{action}</p></div></div><Link to={actionTo} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-cyan-300 px-4 py-2.5 text-[10px] font-black uppercase tracking-[0.14em] text-slate-950 shadow-[0_10px_30px_rgba(34,211,238,.16)] transition hover:-translate-y-0.5 hover:bg-cyan-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/70">{hasSystem ? "Start now" : "Set up habits"}<ArrowRight className="h-3.5 w-3.5" /></Link></div>
  </motion.section>;
}
...
export function DashboardHabits({ habits, onToggle }: { habits: Habit[]; onToggle: (id: string) => void }) {
  ...
  return <section className={`${panel} p-5 sm:p-6`}><div className="flex items-end justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.22em] text-slate-600">Your rhythm</p><h2 className="mt-1 text-xl font-black text-white">Habits that you chose</h2></div><span className="rounded-full border border-white/[0.07] bg-white/[0.03] px-3 py-1.5 text-[10px] font-bold text-slate-500">{habits.length} active</span></div>
    {habits.length === 0 ? <div className="mt-5 rounded-2xl border border-dashed border-cyan-300/15 bg-cyan-300/[0.025] p-5"><div className="grid h-10 w-10 place-items-center rounded-xl border border-cyan-300/15 bg-cyan-300/[0.06] text-cyan-200"><Sparkles className="h-4 w-4" /></div><h3 className="mt-4 text-base font-black text-white">Start with what matters to you.</h3><p className="mt-2 max-w-md text-sm leading-6 text-slate-500">Choose a few habits from the Outstand system. The dashboard will use them to shape recommendations instead of forcing a generic routine on you.</p><Link to="/onboarding" className="mt-4 inline-flex items-center gap-2 rounded-xl border border-cyan-300/20 bg-cyan-300/[0.06] px-3.5 py-2 text-xs font-bold text-cyan-100 transition hover:bg-cyan-300/[0.1] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/70">Choose habits <ArrowRight className="h-3.5 w-3.5" /></Link></div> : <div className="mt-5 grid gap-2 sm:grid-cols-2">{habits.map((habit, index) => { const done = habit.history?.includes(today); return <button key={habit.id} type="button" onClick={() => onToggle(habit.id)} aria-pressed={done} aria-label={`${done ? "Unmark" : "Mark"} ${habit.name} ${done ? "as incomplete" : "as complete"}`} className={`group flex items-center gap-3 rounded-2xl border p-3 text-left transition ${done ? "border-emerald-300/15 bg-emerald-300/[0.045]" : "border-white/[0.07] bg-black/10 hover:border-cyan-300/15 hover:bg-white/[0.035]"}`}><span className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl border text-base ${initialsColor(index)}`}>{habit.emoji}</span><span className="min-w-0 flex-1"><span className={`block truncate text-sm font-bold ${done ? "text-emerald-200" : "text-white"}`}>{habit.name}</span><span className="mt-1 block text-[9px] font-black uppercase tracking-[0.16em] text-slate-600">{done ? "Completed today" : "Mark complete"}</span></span><span className={`grid h-7 w-7 place-items-center rounded-lg border ${done ? "border-emerald-300/20 bg-emerald-300/10 text-emerald-300" : "border-white/10 text-slate-700 group-hover:text-cyan-300"}`}>{done ? <Check className="h-4 w-4" /> : <span className="h-1.5 w-1.5 rounded-full bg-current" />}</span></button>; })}</div>}
  </section>;
}
...
export function DashboardRoadmap({ habits, missions, nextMission, onCompleteMission }: { habits: Habit[]; missions: DashboardMission[]; nextMission?: DashboardMission; onCompleteMission: (id: string) => void }) {
  ...
  return <section className={`${panel} p-5 sm:p-6`}><div className="flex items-end justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.22em] text-slate-600">Your path today</p><h2 className="mt-1 text-xl font-black text-white">A few meaningful moves</h2></div><span className="text-xs font-bold text-cyan-300">{done}/{missions.length}</span></div>
    <div className="mt-5 space-y-2">{missions.slice(0, 4).map((mission, index) => <div key={mission.id} className={`flex items-start gap-3 rounded-2xl border p-3 ${mission.completed ? "border-emerald-300/10 bg-emerald-300/[0.025]" : "border-white/[0.07] bg-black/10"}`}><button type="button" onClick={() => onCompleteMission(mission.id)} disabled={mission.completed || mission.mutating} aria-label={`${mission.completed ? "Completed" : "Complete"} mission: ${mission.title}`} className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl border ${mission.completed ? "border-emerald-300/20 bg-emerald-300/10 text-emerald-300" : "border-white/10 text-slate-600 hover:border-cyan-300/20 hover:text-cyan-300"}`}>{mission.completed ? <Check className="h-4 w-4" /> : <span className="text-[10px] font-black">{index + 1}</span>}</button><div className="min-w-0 flex-1"><p className={`text-sm font-bold leading-5 ${mission.completed ? "text-slate-500 line-through" : "text-white"}`}>{mission.title}</p><p className="mt-1 text-[9px] font-black uppercase tracking-[0.15em] text-slate-600">{mission.category} · +{mission.xpReward} XP</p></div>{mission.mutating ? <span className="shrink-0 rounded-full bg-cyan-300/10 px-2 py-1 text-[9px] font-bold text-cyan-300">Saving</span> : mission.completed ? <span className="shrink-0 rounded-full bg-emerald-300/10 px-2 py-1 text-[9px] font-bold text-emerald-300">Done</span> : null}</div>)}</div>
