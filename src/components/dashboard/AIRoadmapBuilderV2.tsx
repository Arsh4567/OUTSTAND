  ...
  async function request(mode: "questions" | "plan") {
    ...
    try {
      ...
      if (mode === "questions") {
        ...
        const { error: saveError } = await supabase.from("ai_roadmaps").insert({
          ...
          answers,
          plan: nextPlan,
          habits_snapshot: habits.map((h) => ({ id: h.id, name: h.name, emoji: h.emoji })),
          is_active: true,
        });
