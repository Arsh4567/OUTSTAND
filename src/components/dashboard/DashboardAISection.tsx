        ...
        {!loadingSavedPlan && plan && <div className="mt-5 space-y-4">
          <div className="rounded-2xl border border-cyan-300/10 bg-cyan-300/[0.025] p-4 sm:p-5">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div><div className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-cyan-300" /><p className="text-sm font-black text-white">{plan.title}</p></div><p className="mt-1 text-xs leading-5 text-slate-500">Day {roadmapProgress?.day ?? 1} of {plan.durationDays} · {completion}% complete</p></div>
              <button type="button" onClick={() => setBuilderOpen(true)} className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-[0.16em] text-cyan-300 hover:text-cyan-200">Adjust <TrendingUp className="h-3.5 w-3.5" /></button>
            </div>
            <div className="mt-4 h-1.5 overflow-hidden rounded-full bg-white/5"><div className="h-full rounded-full bg-gradient-to-r from-cyan-400 to-violet-400 transition-all" style={{ width: `${Math.min(100, Math.max(0, completion))}%` }} /></div>
          </div>
