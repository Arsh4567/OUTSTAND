...
export function DashboardAISection({ habits, sessions, completedHabits, focusMinutes, bestStreak, nextMission, name, level, xp, roadmapProgress, missions, onCompleteMission }: { habits: Habit[]; sessions: FocusSession[]; completedHabits: number; focusMinutes: number; bestStreak: number; nextMission?: DashboardMission; name: string; level: number; xp: number; roadmapProgress: RoadmapProgress | null; missions: DashboardMission[]; onCompleteMission: (id: string) => void }) {
  ...
  return <>
    <section className={`${panel} p-5 sm:p-6`}>
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div className="max-w-2xl"><div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.2em] text-cyan-300"><span className="grid h-6 w-6 place-items-center rounded-lg border border-cyan-300/15 bg-cyan-300/[0.05]"><Brain className="h-3 w-3" /></span>OUTSTAND Intelligence</div><h2 className="mt-3 text-xl font-black tracking-tight text-white sm:text-2xl">Your plan, hour by hour.</h2><p className="mt-1.5 text-sm leading-6 text-slate-400">A short list of actions with real times, built around your goal and progress.</p></div><button type="button" onClick={() => setBuilderOpen(true)} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-lg bg-cyan-300 px-3.5 py-2 text-[10px] font-black uppercase tracking-[0.12em] text-slate-950 transition hover:bg-cyan-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/70"><Sparkles className="h-3.5 w-3.5" />{plan ? "Adjust schedule" : "Build my plan"}</button></div>
      ...
      {!loadingSavedPlan && !plan && <div className="mt-4 rounded-xl border border-cyan-300/10 bg-cyan-300/[0.02] p-4"><div className="flex items-start gap-3"><BookOpen className="mt-0.5 h-4 w-4 shrink-0 text-cyan-300" /><div><p className="text-sm font-black text-white">No daily plan yet.</p><p className="mt-1 text-xs leading-5 text-slate-500">Tell the AI your goal, deadline and available hours to build a practical timetable.</p></div></div></div>}
      {!loadingSavedPlan && plan && <div className="mt-4 space-y-3"><div className="rounded-xl border border-cyan-300/10 bg-cyan-300/[0.02] p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-cyan-300" /><p className="text-sm font-black text-white">{plan.title}</p></div><p className="mt-1 text-xs leading-5 text-slate-500">Day {roadmapProgress?.day ?? 1} of {plan.durationDays} · {completion}% complete</p></div><button type="button" onClick={() => setBuilderOpen(true)} className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-[0.14em] text-cyan-300 hover:text-cyan-200">Adjust <TrendingUp className="h-3 w-3" /></button></div><div className="mt-3 h-1 overflow-hidden rounded-full bg-white/5"><div className="h-full rounded-full bg-cyan-300 transition-[width] duration-500" style={{ width: `${Math.min(100, Math.max(0, completion))}%` }} /></div></div><DashboardExecutionPlan missions={missions} roadmapProgress={roadmapProgress} timetable={timetable} onCompleteMission={onCompleteMission} />{adapting || adaptationNote ? <div className="rounded-lg border border-violet-300/10 bg-violet-300/[0.025] px-3 py-2 text-[10px] font-semibold text-violet-200/80">{adapting ? "AI is tuning your next schedule from today's progress…" : `Updated: ${adaptationNote}`}</div> : null}</div>}
    </section>
