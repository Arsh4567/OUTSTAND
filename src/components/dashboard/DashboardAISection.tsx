...
export function DashboardAISection({ habits, sessions, completedHabits, focusMinutes, bestStreak, nextMission, name, level, xp, roadmapProgress, missions, onCompleteMission }: { habits: Habit[]; sessions: FocusSession[]; completedHabits: number; focusMinutes: number; bestStreak: number; nextMission?: DashboardMission; name: string; level: number; xp: number; roadmapProgress: RoadmapProgress | null; missions: DashboardMission[]; onCompleteMission: (id: string) => void }) {
  ...
  return <>
    <section className={`${panel} p-5 sm:p-6`}>
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div className="max-w-2xl"><div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.2em] text-cyan-300"><span className="grid h-6 w-6 place-items-center rounded-lg border border-cyan-300/15 bg-cyan-300/[0.05]"><Brain className="h-3 w-3" /></span>OUTSTAND Intelligence</div><h2 className="mt-3 text-xl font-black tracking-tight text-white sm:text-2xl">Your next moves.</h2><p className="mt-1.5 text-sm leading-6 text-slate-400">A focused plan built around your goal, time and progress—not a giant checklist.</p></div><button type="button" onClick={() => setBuilderOpen(true)} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-lg bg-cyan-300 px-3.5 py-2 text-[10px] font-black uppercase tracking-[0.12em] text-slate-950 transition hover:bg-cyan-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/70"><Sparkles className="h-3.5 w-3.5" />{plan ? "Adjust plan" : "Build my plan"}</button></div>
