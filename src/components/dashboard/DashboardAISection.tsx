...
const storageKey = "outstand-ai-roadmap-v1";
...
export function DashboardAISection({ habits, sessions, completedHabits, focusMinutes, bestStreak, nextMission, name, level, xp }: { habits: Habit[]; sessions: FocusSession[]; completedHabits: number; focusMinutes: number; bestStreak: number; nextMission?: DashboardMission; name: string; level: number; xp: number }) {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) setPlan(JSON.parse(saved) as AIRoadmapPlan);
    } catch { /* ignore malformed local roadmap */ }
  }, []);
  ...
  return <>
    <section className={`${panel} relative overflow-hidden p-5 sm:p-7`}>
      ...
      <div className="relative">
        ...
        {plan && <div className="mt-5 rounded-2xl border border-cyan-300/10 bg-cyan-300/[0.025] p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-cyan-300" /><p className="text-sm font-black text-white">{plan.title}</p></div><p className="mt-1 text-xs leading-5 text-slate-500">{plan.durationDays} days · {plan.difficulty} · {plan.milestones.length} milestones</p></div><button type="button" onClick={() => setBuilderOpen(true)} className="text-left text-[10px] font-black uppercase tracking-[0.16em] text-cyan-300 hover:text-cyan-200">Open & refine →</button></div></div>}
      </div>
    </section>
