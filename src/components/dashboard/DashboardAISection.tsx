...
export function DashboardAISection({ habits, sessions, completedHabits, focusMinutes, bestStreak, nextMission, name, level, xp, roadmapProgress, missions, onCompleteMission }: { habits: Habit[]; sessions: FocusSession[]; completedHabits: number; focusMinutes: number; bestStreak: number; nextMission?: DashboardMission; name: string; level: number; xp: number; roadmapProgress: RoadmapProgress | null; missions: DashboardMission[]; onCompleteMission: (id: string) => void }) {
  const [builderOpen, setBuilderOpen] = useState(false); const [plan, setPlan] = useState<DashboardRoadmapPlan | null>(null); const [loadingSavedPlan, setLoadingSavedPlan] = useState(true); const [adapting, setAdapting] = useState(false); const [adaptationNote, setAdaptationNote] = useState("");
  useEffect(() => { let cancelled = false; async function loadSavedRoadmap() {
    ...
    if (cancelled) return; if (error) console.warn("[OUTSTAND] Could not load saved AI roadmap:", error.message);
    const savedPlan = data?.plan; if (savedPlan && typeof savedPlan === "object" && !Array.isArray(savedPlan)) setPlan(savedPlan as unknown as DashboardRoadmapPlan); setLoadingSavedPlan(false); if (!savedPlan) return;
    const adaptationKey = `outstand-roadmap-adapted:${localDateKey()}`; if (localStorage.getItem(adaptationKey) === "1") return;
    const { data: sessionData } = await supabase.auth.getSession(); const token = sessionData.session?.access_token; if (!token) return; localStorage.setItem(adaptationKey, "1"); setAdapting(true);
    try { const response = await fetch("/api/roadmap", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ mode: "adapt", category: "custom", answers: {}, habits: habits.map((h) => ({ id: h.id, name: h.name, emoji: h.emoji })), context: { name, level, xp, streak: bestStreak, focusMinutes, completedHabits, today: localDateKey() } }) }); const result = await response.json().catch(() => ({})); if (!cancelled && response.ok && result.plan && typeof result.plan === "object") { setPlan(result.plan as DashboardRoadmapPlan); if (result.changed && result.reason) setAdaptationNote(result.reason); } if (!response.ok) localStorage.removeItem(adaptationKey); }
  ...
  } void loadSavedRoadmap(); return () => { cancelled = true; }; }, [bestStreak, habits, level, name, xp, focusMinutes, completedHabits]);

  const timetable = plan?.dailySchedule?.filter((block) => block?.title && block?.instructions).slice(0, 6).map((block) => ({ label: `${block.startTime}–${block.endTime}`, durationMinutes: block.estimatedMinutes, task: block.title, why: block.instructions })) ?? [];
  ...
  return <>
    <section className={`${panel} relative overflow-hidden p-5 sm:p-7`}>
      ...
      <div className="relative">
        ...
        {!loadingSavedPlan && plan && <div className="mt-5 space-y-4"><div className="rounded-2xl border border-cyan-300/10 bg-cyan-300/[0.025] p-4 sm:p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-cyan-300" /><p className="text-sm font-black text-white">{plan.title}</p></div><p className="mt-1 text-xs leading-5 text-slate-500">Day {roadmapProgress?.day ?? 1} of {plan.durationDays} · {completion}% complete</p></div><button type="button" onClick={() => setBuilderOpen(true)} className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-[0.16em] text-cyan-300 hover:text-cyan-200">Adjust <TrendingUp className="h-3.5 w-3.5" /></button></div><div className="mt-4 h-1.5 overflow-hidden rounded-full bg-white/5"><div className="h-full rounded-full bg-gradient-to-r from-cyan-400 to-violet-400 transition-all" style={{ width: `${Math.min(100, Math.max(0, completion))}%` }} /></div></div><DashboardExecutionPlan missions={missions} roadmapProgress={roadmapProgress} timetable={timetable} onCompleteMission={onCompleteMission} />{adapting || adaptationNote ? <div className="rounded-xl border border-violet-300/10 bg-violet-300/[0.035] px-3 py-2 text-[10px] font-semibold text-violet-200/80">{adapting ? "AI is tuning your next schedule from today's progress…" : `Updated: ${adaptationNote}`}</div> : null}</div>}
      </div>
    </section>
