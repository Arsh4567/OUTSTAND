...
export function useDashboard() {
  ...
  const applyStats = useCallback((data: Stats | null) => {
    ...
    setSnapshot((prev) => ({
      ...
      streak: Math.max(0, Number(data?.streak_days ?? 0)),
      xpPct: levelState.progressPct,
    }));
  }, []);
  ...
  const loadRoadmapProgress = useCallback(async () => {
    ...
    if (error) {
      console.warn("Roadmap progress refresh failed:", error.message);
      return null;
    }
  ...
  const load = useCallback(async (signal?: AbortSignal) => {
    ...
    const { error: missionSyncError } = await supabase.rpc("sync_ai_roadmap_today_missions" as never);
    if (missionSyncError) console.warn("Today's roadmap missions could not be refreshed:", missionSyncError.message);
    if (signal?.aborted) return;
  ...
  useEffect(() => {
    ...
    load(controller.signal).catch((error) => {
      if (!controller.signal.aborted) {
        ...
        setLoadError(error instanceof Error ? error.message : "We couldn't load your dashboard data.");
      }
    }).finally(() => {
