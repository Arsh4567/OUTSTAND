...
export function useAppState() {
  ...
  const [xp, setXp] = useState(0), [currentStreak, setCurrentStreak] = useState(0), [cloudBestStreak, setCloudBestStreak] = useState(0), [xpLoading, setXpLoading] = useState(true);
  ...
  // Avoid an auth lookup for every profile UPDATE. The realtime path only needs the
  // current user's id, so caching it keeps frequent XP events O(1) and network-free.
  useEffect(() => {
    let active = true;
    let userId: string | null = null;
    void supabase.auth.getUser().then(({ data: { user } }) => { if (active) userId = user?.id ?? null; });
    const subscription = supabase.auth.onAuthStateChange((_event, session) => { userId = session?.user?.id ?? null; });
    const channel = supabase.channel("live-xp").on("postgres_changes", { event: "UPDATE", schema: "public", table: "profiles" }, payload => {
      if (!active || !userId || userId !== payload.new.id) return;
      setXp(Math.max(0, Number(payload.new.total_xp) || 0));
    }).subscribe();
    return () => { active = false; subscription.data.subscription.unsubscribe(); void supabase.removeChannel(channel); };
  }, []);
  ...
  const awardXp = useCallback(async (amount: number, source: string, referenceId: string, description: string) => { const safeAmount = Math.max(0, Math.round(Number.isFinite(amount) ? amount : 0)); if (!safeAmount) return 0; const { data, error } = await supabase.rpc("award_xp", { p_amount: safeAmount, p_source: source, p_reference_id: referenceId, p_description: description }); if (error) return 0; const granted = Number(data?.[0]?.granted) || 0; if (data?.[0]?.total_xp != null) setXp(Number(data[0].total_xp) || 0); return granted; }, []);
  ...
  const toggleToday = (id: string) => { const today = todayISO(); const habit = habits.find(item => item.id === id); if (!habit) return; const history = Array.isArray(habit.history) ? habit.history : []; const alreadyDone = history.includes(today); setHabits(prev => (Array.isArray(prev) ? prev : []).map(item => item.id === id ? { ...item, history: alreadyDone ? history.filter(day => day !== today) : [...history, today] } : item)); if (!alreadyDone) void awardXp(10, "habit", `${id}:${today}`, "Daily habit completed"); markTodayActive(); };
  const addHabit = (data: { name: string; emoji: string; color: string }) => { const name = data.name.trim(), normalized = normalizeHabitName(name); if (!normalized) return; setHabits(prev => { const current = Array.isArray(prev) ? prev : []; if (current.some(habit => normalizeHabitName(habit.name) === normalized)) return current; return [...current, { id: safeUuid(), name, emoji: data.emoji || "✨", color: data.color || "primary", createdAt: new Date().toISOString(), history: [] }]; }); };
