...
export function useAppState() {
  ...
  const addHabit=(data:{name:string;emoji:string;color:string;notificationTime?:string|null})=>{const name=data.name.trim();const normalized=normalizeHabitName(name);if(!normalized)return false;let added=false;setHabits(prev=>{const current=Array.isArray(prev)?prev:[];if(current.length>=MAX_HABITS||current.some(habit=>normalizeHabitName(habit.name)===normalized))return current;added=true;return[...current,{id:safeUuid(),name,emoji:data.emoji||"✨",color:data.color||"ink",notificationTime:data.notificationTime||null,createdAt:new Date().toISOString(),history:[]}]});return added;};
