...
export function useAppState() {
  ...
  const refreshCloudStreak = useCallback(async (recordToday = false) => {
    try {
      const { supabase } = await import("@/integrations/supabase/client"); const { data: { user } } = await supabase.auth.getUser();
