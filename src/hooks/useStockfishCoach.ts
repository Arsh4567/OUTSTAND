...
export function classifyMove(delta: number | null, matchedBest: boolean, missedMate = false): MoveClassification { if (missedMate) return "blunder"; if (matchedBest) return "best"; if (delta == null) return "good"; if (delta <= 10) return "excellent"; if (delta <= 30) return "good"; if (delta <= 80) return "inaccuracy"; if (delta <= 200) return "mistake"; return "blunder"; }
...
export function useStockfishCoach() {
  ...
  const analyze = useCallback(async (fen: string, depth = 14): Promise<CoachEvaluation> => {
    ...
    return new Promise<CoachEvaluation>((resolve, reject) => {
      ...
      worker.addEventListener("message", onMessage); worker.postMessage("isready"); worker.postMessage("ucinewgame"); worker.postMessage(`position fen ${fen}`); worker.postMessage(`go depth ${Math.max(12, Math.min(15, depth))}`);
    });
  }, [ensureReady]);
  const analyzeMove = useCallback(async (beforeFen: string, afterFen: string, side: "w" | "b", playedMove: string, depth = 14): Promise<CoachAnalysis> => { const before = await analyze(beforeFen, depth); const after = await analyze(afterFen, depth); const bestPerspective = perspective(before.score, side); const afterPerspective = perspective(after.score, side); const delta = bestPerspective != null && afterPerspective != null ? Math.max(0, bestPerspective - afterPerspective) : null; const matchedBest = !!before.bestMove && before.bestMove.toLowerCase() === playedMove.toLowerCase(); const missedMate = before.mate != null && before.mate > 0 && after.mate == null; return { before, after, classification: classifyMove(delta, matchedBest, missedMate), delta, bestMove: before.bestMove, pv: before.pv }; }, [analyze]);
