...
export function useRoadmap() {
  ...
  const toggleTask = useCallback(async (task: RoadmapTask) => {
    ...
    const { error } = await supabase.from("roadmap_task_progress").upsert({ task_id: task.id, roadmap_id: roadmap.id, user_id: roadmap.user_id, status: nextStatus, completed_at: nextStatus === "completed" ? new Date().toISOString() : null }, { onConflict: "task_id,user_id" });
    if (error) throw error;
