...
export function useRoadmap() {
  ...
  const invoke = useCallback(async <T = ActionResponse>(action: string, body: Record<string, unknown> = {}): Promise<T> => {
    ...
    if (error) {
      ...
      try { if (response && typeof response.json === "function") { const payload = await response.json(); if (payload?.error) message = String(payload.error); } } catch { /* ignore unreadable edge-function payload */ }
  ...
  const updateRoadmap = useCallback(async (roadmapId: string, patch: { title: string; goal: string }) => {
    ...
    const { data: { user }, error: userError } = await supabase.auth.getUser(); if (userError) throw userError; if (!user) throw new Error("Please sign in again.");
    ...
    if (error) throw error;
    setRoadmap(data as RoadmapSummary); setRoadmaps((items) => items.map((item) => item.id === roadmapId ? { ...item, ...(data as RoadmapSummary) } : item)); return data as RoadmapSummary;
  }, []);

  const smartChange = useCallback(async (roadmapId: string, request: string) => { if (request.trim().length < 5) throw new Error("Describe the change you want to make."); return await invoke("smart_change", { roadmapId, request }); }, [invoke]);
  const deleteRoadmap = useCallback(async (roadmapId: string) => { const data = await invoke("delete_roadmap", { roadmapId }); const next = roadmaps.filter((item) => item.id !== roadmapId); setRoadmaps(next); if (roadmap?.id === roadmapId) { setRoadmap(next[0] || null); setMilestones([]); setTasks([]); if (next[0]) await load(next[0].id); } return data; }, [invoke, load, roadmap, roadmaps]);
  ...
  const generate = useCallback(async (category: string, currentAnswers: Record<string, unknown>) => { setGenerating(true); try { const data = await invoke("generate_roadmap", { category, answers: currentAnswers }); if (data?.needsMoreInfo) { const next = normalizeQuestions(data.questions); const visible = next.length ? next : [fallbackFollowUp]; setQuestions(visible); return { ...data, questions: visible }; } if (!data?.roadmapId) throw new Error("Roadmap was generated but no saved roadmap ID was returned."); setAnswers(currentAnswers); await load(data.roadmapId); return data; } catch (error) { toast.error(error instanceof Error ? error.message : "Could not generate roadmap."); throw error; } finally { setGenerating(false); } }, [invoke, load]);
  ...
  const todayIndex = useMemo(() => { if (!roadmap?.start_date) return 1; const start = new Date(`${roadmap.start_date}T00:00:00`), now = new Date(); return Math.max(1, Math.floor((new Date(now.getFullYear(), now.getMonth(), now.getDate()).getTime() - start.getTime()) / 86400000) + 1); }, [roadmap?.start_date]);
