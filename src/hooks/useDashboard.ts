...
export type DashboardSnapshot = { userName: string; totalXp: number; level: number; streak: number; xpPct: number; missions: DashboardMission[]; completedCount: number; completionPct: number; quote: { quote: string; author: string } };
...
export function useDashboard() {
  const [snapshot, setSnapshot] = useState<DashboardSnapshot>({ userName: "there", totalXp: 0, level: 1, streak: 0, xpPct: 0, missions: [], completedCount: 0, completionPct: 0, quote: quoteOfTheDay() });
  ...
  const load = useCallback(async (signal?: AbortSignal) => {
    ...
    setSnapshot((prev) => ({ ...prev, missions: mapped }));
  }, [applyStats, ensureStats]);
  ...
  const derived = useMemo(() => { const completedCount = snapshot.missions.filter((m) => m.completed).length; const completionPct = snapshot.missions.length ? Math.round((completedCount / snapshot.missions.length) * 100) : 0; return { ...snapshot, completedCount, completionPct }; }, [snapshot]);
  return { snapshot: derived, isLoading, loadError };
}
